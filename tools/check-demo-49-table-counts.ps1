...
Set-Location $root
$sql = Join-Path $PSScriptRoot 'check-demo-49-table-counts.sql'
$remoteSql = '/tmp/cinebooking-check-49-tables.sql'
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Could not inspect 49 table row counts." }
}
finally {
