...
Write-Host "=== CineBooking V48: realistic reference data across all 52 pgAdmin tables ===" -ForegroundColor Cyan
...
Write-Host "V48 multi-cinema concessions: branch stock and branch prices are refreshed for all 10 reference cinemas; V47 payment history remains covered." -ForegroundColor Cyan
...
$sql = Join-Path $PSScriptRoot 'seed-demo-52-tables-10-rows.sql'
if (-not (Test-Path $sql)) {
    throw "Seed SQL not found: $sql"
}

$remoteSql = '/tmp/cinebooking-reference52-utf8.sql'
try {
...
Write-Host "All 52 pgAdmin tables now have data; Flyway history remains genuine." -ForegroundColor Green
