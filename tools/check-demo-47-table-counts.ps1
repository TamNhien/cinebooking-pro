...
Set-Location $root
$sql = Join-Path $PSScriptRoot 'check-demo-47-table-counts.sql'
$remoteSql = '/tmp/cinebooking-check-47-tables.sql'
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Could not inspect 47 table row counts." }
}
finally {
