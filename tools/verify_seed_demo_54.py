...
sql_path = root / 'tools' / 'seed-demo-54-tables-10-rows.sql'
ps1_path = root / 'tools' / 'seed-demo-54-tables.ps1'
check_sql = root / 'tools' / 'check-demo-54-table-counts.sql'

# V50 has 53 application tables plus Flyway metadata = 54 tables in pgAdmin.
...
expected_seed_inserts = {
    ...
    'password_reset_token','payment','payment_event','payment_webhook_event','pricing_rule','recommendation_event','recommendation_feedback',
...
all_54_tables = expected_seed_inserts | {'movie','flyway_schema_history'}
canonical_movie_ids = [
...
verification_array = set(re.findall(r"'([a-z_]+)'", sql[sql.find('Quick verification of all 54 tables'):]))
main_data = sql[:sql.find('-- Fail if seeded reference rows')]

check('V50 54-table SQL exists and decodes as UTF-8', sql_path.exists() and 'Nguyễn Minh An' in sql and 'Bắp Phô Mai Lớn' in sql)
check('V50 54-table PowerShell runner exists', ps1_path.exists())
check('All 52 seeded application tables have INSERT coverage', expected_seed_inserts.issubset(targets) and len(expected_seed_inserts) == 52)
...
check('Final verification enumerates all 54 pgAdmin tables', all_54_tables.issubset(verification_array) and len(all_54_tables) == 54)
check('Row-count helper includes V46/V47/V48/V49/V50 tables', counts.count("'trusted_device'") == 2 and counts.count("'security_alert'") == 2 and counts.count("'payment_event'") == 2 and counts.count("'cinema_concession_inventory'") == 2 and counts.count("'cinema_concession_price'") == 2 and counts.count("'showtime_planning_run'") == 2 and counts.count("'recommendation_feedback'") == 2)
...
print(f"\nSeed V50 54-table realistic-data verification: {passed}/{len(checks)} checks passed")
