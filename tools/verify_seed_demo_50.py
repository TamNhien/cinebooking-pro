...
sql_path = root / 'tools' / 'seed-demo-50-tables-10-rows.sql'
ps1_path = root / 'tools' / 'seed-demo-50-tables.ps1'
check_sql = root / 'tools' / 'check-demo-50-table-counts.sql'

# V47 has 49 application tables plus Flyway metadata = 50 tables in pgAdmin.
...
all_50_tables = expected_seed_inserts | {'movie','flyway_schema_history'}
canonical_movie_ids = [
...
verification_array = set(re.findall(r"'([a-z_]+)'", sql[sql.find('Quick verification of all 50 tables'):]))
main_data = sql[:sql.find('-- Fail if seeded reference rows')]

check('V47 50-table SQL exists and decodes as UTF-8', sql_path.exists() and 'Nguyễn Minh An' in sql and 'Bắp Phô Mai Lớn' in sql)
check('V47 50-table PowerShell runner exists', ps1_path.exists())
check('All 48 seeded application tables have INSERT coverage', expected_seed_inserts.issubset(targets) and len(expected_seed_inserts) == 48)
...
check('V47 payment_event receives 10 honest local history markers', 'INSERT INTO payment_event(' in sql and 'seed47:payment-event:' in sql and 'PAYMENT_HISTORY_IMPORTED' in sql and 'V47_REFERENCE_SYNC' in sql)
...
check('Final verification enumerates all 50 pgAdmin tables', all_50_tables.issubset(verification_array) and len(all_50_tables) == 50)
check('Row-count helper includes V46/V47 tables', counts.count("'trusted_device'") == 2 and counts.count("'security_alert'") == 2 and counts.count("'payment_event'") == 2)
...
print(f"\nSeed V47 50-table realistic-data verification: {passed}/{len(checks)} checks passed")
