...
Write-Host "=== CineBooking V50: realistic reference data across all 54 pgAdmin tables ===" -ForegroundColor Cyan
...
Write-Host "V50 recommendation intelligence: 10 explicit taste-feedback rows are seeded; V49 planner, V48 inventory and V47 payment history remain covered." -ForegroundColor Cyan
...
$sql = Join-Path $PSScriptRoot 'seed-demo-54-tables-10-rows.sql'
if (-not (Test-Path $sql)) { throw "Seed SQL not found: $sql" }

$remoteSql = '/tmp/cinebooking-reference54-utf8.sql'
try {
...
Write-Host "All 54 pgAdmin tables now have data; Flyway history remains genuine." -ForegroundColor Green
