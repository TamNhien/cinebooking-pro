...
controlled = text("frontend/lib/controlled-business-presentation.ts")

ok("usePresentationLanguage" in page and re.search(r"const\s*\{[^}]*\bt\b[^}]*\}\s*=\s*usePresentationLanguage\(\);", page),
   "Admin Audit owns presentation language directly, including forward-compatible language context")
...
ok(page.count('data-i18n-skip="true"') >= 5 and all(x in page for x in [
    'x.actorEmail || "system"', 'x.action', 'x.entityType || "—"', 'x.ipAddress || "—"'
]) and 'auditDetailPresentation(x.details,language) || "—"' in page
   and 'export function auditDetailPresentation' in controlled and 'return raw;' in controlled,
   "Audit actor/action/entity/IP stay source-owned while details use bounded system presentation with raw fallback")
...
prev4_name = "tools/verify_v78_0_4_notification_presentation_language_business_boundaries.py"
prev4 = text(prev4_name)
ok(bool(prev4) and "V78.0.4 notification presentation-language/business-data boundary verification" in prev4
   and prev4_name in release and prev4_name in ci and prev4_name in diag,
   "V78.0.4 remains an explicit earlier release/CI/diagnostic gate without recursive preflight replay")
