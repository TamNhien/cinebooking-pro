...
check("Unit test validates per-file report metadata", all(x in test for x in ["Khoảng dữ liệu,30 ngày", "Rạp,Tất cả rạp", "Ngày xuất"]))
check("Unit test validates detailed daily revenue CSV", "02-doanh-thu-theo-ngay.csv" in test and "Ngày,Doanh thu,Booking,Vé,Check-in" in test)
check("Unit test validates detailed payment provider CSV", "12-phuong-thuc-thanh-toan.csv" in test and "Provider,Doanh thu,Giao dịch" in test)
