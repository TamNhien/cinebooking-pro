...
Write-Host '=== CineBooking V54 source diagnostics ===' -ForegroundColor Cyan
$checks=@(
...
foreach($check in $checks){
  ...
  python $check
  if($LASTEXITCODE -ne 0){throw "V54 diagnostic failed: $check"}
}
Write-Host "`nV54 source diagnostics passed." -ForegroundColor Green
