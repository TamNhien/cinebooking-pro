...
check('Integration test keeps V50 schema coverage on V50-or-newer Flyway catalog', 'recommendation_feedback' in integration and (('flywayMigratesRealPostgresToV50RecommendationIntelligenceSchemaAndCatalog' in integration and 'assertThat(latest).isEqualTo("50")' in integration) or ('flywayMigratesRealPostgresToV51AnalyticsForecastingSchemaAndCatalog' in integration and 'assertThat(latest).isEqualTo("51")' in integration)))
check('Main CI retains V50 source and 54-table compatibility gates', ('V26-V50 source regression' in ci or 'V26-V51 source regression' in ci) and 'verify_v50_recommendation_intelligence_2.py' in ci and 'verify_seed_demo_54.py' in ci)
check('Standalone RC keeps V50 gate in V50-or-newer candidate', ('v50.0.0-rc.1' in rc or 'v51.0.0-rc.1' in rc) and ('cinebooking_v50_rc_' in rc or 'cinebooking_v51_rc_' in rc) and 'verify_v50_recommendation_intelligence_2.py' in rc)
check('Stable release keeps V50 gate in V50-or-newer release', ('default: "50.0.0"' in release or 'default: "51.0.0"' in release) and ('cinebooking_v50_release_' in release or 'cinebooking_v51_release_' in release) and 'verify_v50_recommendation_intelligence_2.py' in release)
