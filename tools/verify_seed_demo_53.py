...
sql_path = root / 'tools' / 'seed-demo-53-tables-10-rows.sql'
ps1_path = root / 'tools' / 'seed-demo-53-tables.ps1'
check_sql = root / 'tools' / 'check-demo-53-table-counts.sql'

# V49 has 52 application tables plus Flyway metadata = 53 tables in pgAdmin.
...
expected_seed_inserts = {
    ...
    'password_reset_token','payment','payment_event','payment_webhook_event','pricing_rule','recommendation_event',
...
all_53_tables = expected_seed_inserts | {'movie','flyway_schema_history'}
canonical_movie_ids = [
...
verification_array = set(re.findall(r"'([a-z_]+)'", sql[sql.find('Quick verification of all 53 tables'):]))
main_data = sql[:sql.find('-- Fail if seeded reference rows')]

check('V49 53-table SQL exists and decodes as UTF-8', sql_path.exists() and 'Nguyễn Minh An' in sql and 'Bắp Phô Mai Lớn' in sql)
check('V49 53-table PowerShell runner exists', ps1_path.exists())
check('All 51 seeded application tables have INSERT coverage', expected_seed_inserts.issubset(targets) and len(expected_seed_inserts) == 51)
...
check('Final verification enumerates all 53 pgAdmin tables', all_53_tables.issubset(verification_array) and len(all_53_tables) == 53)
check('Row-count helper includes V46/V47/V48/V49 tables', counts.count("'trusted_device'") == 2 and counts.count("'security_alert'") == 2 and counts.count("'payment_event'") == 2 and counts.count("'cinema_concession_inventory'") == 2 and counts.count("'cinema_concession_price'") == 2 and counts.count("'showtime_planning_run'") == 2)
...
print(f"\nSeed V49 53-table realistic-data verification: {passed}/{len(checks)} checks passed")
