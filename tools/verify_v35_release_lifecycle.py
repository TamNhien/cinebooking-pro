...
check('stable-release workflow defaults to semantic V35 or newer', re.search(r'default: "(?:3[5-9]|[4-9][0-9])\.\d+\.\d+"', release) is not None)
...
check('RC E2E uses disposable full-stack smoke', 'bash tools/smoke-v29.sh' in release and re.search(r'cinebooking_v(?:3[5-9]|[4-9][0-9])_release_\$\{\{ github\.run_id \}\}', release) is not None)
...
check('standalone RC defaults to semantic V35 or newer candidate', re.search(r'default: "v(?:3[5-9]|[4-9][0-9])\.\d+\.\d+-rc\.1"', rc) is not None)
...
check('README documents feature-to-release lifecycle', 'Release Candidate E2E' in readme and 'GitHub Release' in readme and re.search(r'v(?:3[5-9]|[4-9][0-9])\.\d+\.\d+-rc\.1', readme) is not None and re.search(r'v(?:3[5-9]|[4-9][0-9])\.\d+\.\d+', readme) is not None)
