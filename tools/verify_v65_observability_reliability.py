...
check("CI source regression reaches V65 or later", bool(ci_match) and int(ci_match.group(1))>=65)
...
check("RC defaults to V65 or later", bool(rc_default) and int(rc_default.group(1))>=65)
stable_default=re.search(r'default: "(\d+)\.0\.0"',release)
check("Stable defaults to V65 or later", bool(stable_default) and int(stable_default.group(1))>=65)
check("RC compose project is V65 or later", bool(re.search(r"cinebooking_v(6[5-9]|[7-9][0-9])_rc_",rc)))
check("Stable compose project is V65 or later", bool(re.search(r"cinebooking_v(6[5-9]|[7-9][0-9])_release_",release)))
...
check("README current release is V65 or later", bool(current_match) and int(current_match.group(1))>=65 and "## V65 - Observability & Reliability 4.0" in readme)
