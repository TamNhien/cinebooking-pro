...
-- -----------------------------------------------------------------------------
-- 06B. showtime_planning_run (10) - V49 smart-planner audit history
-- -----------------------------------------------------------------------------
INSERT INTO showtime_planning_run(
    id,cinema_id,movie_id,from_date,to_date,target_per_day,operating_start,operating_end,interval_minutes,base_price,
    requested_slots,suggested_slots,conflict_count,historical_samples,strategy,status,created_by,plan_json,created_at,committed_at
)
SELECT
    md5('seed49:planning-run:' || n)::uuid,
    md5('seed45:cinema:' || n)::uuid,
    (SELECT movie_id FROM seed45_movie_map WHERE seed45_movie_map.n = g.n),
    CURRENT_DATE - (n + 6),
    CURRENT_DATE - (n + 6),
    ...
    6 + n,
    'V49-DEMAND-BALANCED-2',
    'COMMITTED',
    (SELECT email FROM seed_real_people WHERE seed_real_people.n = g.n),
    '[{"decision":"historical-demand","source":"reference-history"}]',
    CURRENT_TIMESTAMP - (n || ' days')::interval,
    CURRENT_TIMESTAMP - (n || ' days')::interval + INTERVAL '5 minutes'
FROM generate_series(1,10) AS g(n)
ON CONFLICT (id) DO UPDATE SET
    cinema_id=EXCLUDED.cinema_id,movie_id=EXCLUDED.movie_id,from_date=EXCLUDED.from_date,to_date=EXCLUDED.to_date,
    target_per_day=EXCLUDED.target_per_day,operating_start=EXCLUDED.operating_start,operating_end=EXCLUDED.operating_end,
    interval_minutes=EXCLUDED.interval_minutes,base_price=EXCLUDED.base_price,requested_slots=EXCLUDED.requested_slots,
    suggested_slots=EXCLUDED.suggested_slots,conflict_count=EXCLUDED.conflict_count,historical_samples=EXCLUDED.historical_samples,
    strategy=EXCLUDED.strategy,status=EXCLUDED.status,created_by=EXCLUDED.created_by,plan_json=EXCLUDED.plan_json,
    created_at=EXCLUDED.created_at,committed_at=EXCLUDED.committed_at;

UPDATE showtime s SET
    planning_source='SMART',
    planning_run_id=md5('seed49:planning-run:' || g.n)::uuid,
    planning_score=LEAST(100, 62 + g.n * 2)
FROM generate_series(1,10) g(n)
WHERE s.id=md5('seed45:showtime:' || g.n)::uuid;
...
-- -----------------------------------------------------------------------------
-- 12A. cinema_concession_inventory (V48 branch stock; all 10 x 10 reference pairs)
-- -----------------------------------------------------------------------------
...
SELECT
    md5('seed48:branch-inventory:' || c.n || ':' || p.n)::uuid,
    md5('seed45:cinema:' || c.n)::uuid,
    md5('seed45:product:' || p.n)::uuid,
    CASE WHEN (c.n + p.n) % 11 = 0 THEN 0 WHEN (c.n + p.n) % 7 = 0 THEN 8 ELSE 120 + ((c.n * 11 + p.n * 7) % 80) END,
    0,
    20,
    180,
    TRUE,
    CURRENT_TIMESTAMP
FROM generate_series(1,10) c(n) CROSS JOIN generate_series(1,10) p(n)
ON CONFLICT (cinema_id,product_id) DO UPDATE SET
    stock_on_hand=EXCLUDED.stock_on_hand,stock_reserved=0,low_stock_threshold=EXCLUDED.low_stock_threshold,
    target_stock=EXCLUDED.target_stock,active=TRUE,updated_at=CURRENT_TIMESTAMP;

-- -----------------------------------------------------------------------------
-- 12B. cinema_concession_price (V48 branch prices; all 10 x 10 reference pairs)
-- -----------------------------------------------------------------------------
INSERT INTO cinema_concession_price(id,cinema_id,product_id,price,active,updated_at)
SELECT
    md5('seed48:branch-price:' || c.n || ':' || p.n)::uuid,
    md5('seed45:cinema:' || c.n)::uuid,
    md5('seed45:product:' || p.n)::uuid,
    (SELECT price FROM seed_real_product WHERE seed_real_product.n=p.n) + (((c.n - 1) % 3) * 2000),
    TRUE,CURRENT_TIMESTAMP
FROM generate_series(1,10) c(n) CROSS JOIN generate_series(1,10) p(n)
ON CONFLICT (cinema_id,product_id) DO UPDATE SET price=EXCLUDED.price,active=TRUE,updated_at=CURRENT_TIMESTAMP;
...
-- -----------------------------------------------------------------------------
-- 17B. payment_event (10) - honest local payment-history import markers
-- -----------------------------------------------------------------------------
INSERT INTO payment_event(
    id,payment_id,event_type,actor_type,actor_ref,from_status,to_status,code,message,details_json,created_at
)
SELECT
    md5('seed47:payment-event:' || n)::uuid,
    md5('seed45:payment:' || n)::uuid,
    'PAYMENT_HISTORY_IMPORTED','MIGRATION',NULL,'SUCCESS','SUCCESS','V47_REFERENCE_SYNC',
    'Đồng bộ lịch sử thanh toán nội bộ',
    '{"provider":"MOCK","source":"reference-sync"}',
    CURRENT_TIMESTAMP - ((n + 1) || ' days')::interval + INTERVAL '6 minutes'
FROM generate_series(1,10) AS g(n)
ON CONFLICT DO NOTHING;
...
INSERT INTO inventory_movement(
    id,product_id,cinema_id,booking_id,movement_type,quantity_delta,reserved_delta,
    stock_after,reserved_after,actor_email,reference_key,note,created_at
)
SELECT
    ...
    md5('seed45:cinema:' || n)::uuid,
    NULL,'RESTOCK',20,0,
    (SELECT stock_on_hand FROM cinema_concession_inventory WHERE cinema_id=md5('seed45:cinema:' || n)::uuid AND product_id=md5('seed45:product:' || n)::uuid),0,
    'an.nguyen@cinebooking.local',
    format('REFERENCE-RESTOCK-%s',to_char(n,'FM00')),
    ...
    CURRENT_TIMESTAMP - (n || ' hours')::interval
FROM generate_series(1,10) AS g(n)
ON CONFLICT (id) DO UPDATE SET cinema_id=EXCLUDED.cinema_id,stock_after=EXCLUDED.stock_after,reference_key=EXCLUDED.reference_key,note=EXCLUDED.note;
...
-- -----------------------------------------------------------------------------
-- 25B. recommendation_feedback (10) - V50 explicit taste controls
-- -----------------------------------------------------------------------------
...
UPDATE inventory_movement i SET cinema_id=md5('seed45:cinema:' || g.n)::uuid, reference_key=format('REFERENCE-RESTOCK-%s',to_char(g.n,'FM00')), actor_email='an.nguyen@cinebooking.local', note=(ARRAY['Nhập bắp caramel','Nhập bắp phô mai','Nhập bắp ngọt','Nhập Coca-Cola','Nhập Sprite','Nhập Fanta cam','Nhập nước suối','Bổ sung Combo Solo','Bổ sung Combo Couple Plus','Bổ sung Combo Family'])[g.n]
...
-- V51A. cinema_concession_cost_basis (10) - deterministic CI/reference cost examples.
-- Only 10 of the 100 cinema/product combinations receive reference cost. Missing combinations stay unknown/NULL by design.
INSERT INTO cinema_concession_cost_basis(id,cinema_id,product_id,unit_cost,source,updated_by,updated_at)
SELECT
    md5('seed51:cost-basis:' || g.n)::uuid,
    md5('seed45:cinema:' || g.n)::uuid,
    md5('seed45:product:' || g.n)::uuid,
    (ARRAY[22000,28000,12000,17000,15000,19000,26000,31000,38000,44000]) [g.n]::numeric(12,2),
    'REFERENCE',
    'an.nguyen@cinebooking.local',
    timestamptz '2026-08-24 09:00:00+07' + ((g.n - 1) * interval '5 minutes')
FROM generate_series(1,10) g(n)
ON CONFLICT (cinema_id,product_id) DO UPDATE SET
...
-- V51B. analytics_snapshot (10) - deterministic CI/reference DAILY/WEEKLY/MONTHLY snapshots.
INSERT INTO analytics_snapshot(
    id,cinema_id,period_kind,period_start,period_end,revenue,ticket_revenue,concession_revenue,
    concession_cost,gross_margin,bookings,tickets,capacity,occupancy_rate,cost_coverage_rate,
    forecast_next_7d,forecast_algorithm,generated_at
)
SELECT
    md5('seed51:analytics-snapshot:' || g.n)::uuid,
    md5('seed45:cinema:' || g.n)::uuid,
    CASE ((g.n - 1) % 3) WHEN 0 THEN 'DAILY' WHEN 1 THEN 'WEEKLY' ELSE 'MONTHLY' END,
    CASE ((g.n - 1) % 3) WHEN 0 THEN date '2026-08-23' WHEN 1 THEN date '2026-08-17' ELSE date '2026-07-01' END,
    CASE ((g.n - 1) % 3) WHEN 0 THEN date '2026-08-23' WHEN 1 THEN date '2026-08-23' ELSE date '2026-07-31' END,
    (1450000 + g.n * 175000)::numeric(14,2),
    (1120000 + g.n * 140000)::numeric(14,2),
    (330000 + g.n * 35000)::numeric(14,2),
    (145000 + g.n * 12000)::numeric(14,2),
    ((1450000 + g.n * 175000) - (145000 + g.n * 12000))::numeric(14,2),
    18 + g.n,
    31 + g.n * 2,
    70 + g.n * 5,
    round(((31 + g.n * 2)::numeric * 100 / (70 + g.n * 5)),3),
    100.000,
    (10800000 + g.n * 420000)::numeric(14,2),
    'V51-WEEKDAY-WEIGHTED-MA-1',
    timestamptz '2026-08-24 09:30:00+07' + ((g.n - 1) * interval '5 minutes')
FROM generate_series(1,10) g(n)
...
BEGIN
    ...
    IF (SELECT COUNT(*) FROM cinema_concession_inventory WHERE cinema_id IN (SELECT md5('seed45:cinema:' || n)::uuid FROM generate_series(1,10) g(n)) AND product_id IN (SELECT md5('seed45:product:' || n)::uuid FROM generate_series(1,10) g(n))) <> 100 THEN
        RAISE EXCEPTION 'V48 branch inventory refresh failed: expected 100 cinema/product rows';
    END IF;
    IF (SELECT COUNT(*) FROM cinema_concession_price WHERE cinema_id IN (SELECT md5('seed45:cinema:' || n)::uuid FROM generate_series(1,10) g(n)) AND product_id IN (SELECT md5('seed45:product:' || n)::uuid FROM generate_series(1,10) g(n))) <> 100 THEN
        RAISE EXCEPTION 'V48 branch pricing refresh failed: expected 100 cinema/product rows';
    END IF;
    IF (SELECT COUNT(*) FROM recommendation_feedback WHERE id IN (SELECT md5('seed50:recommendation-feedback:' || n)::uuid FROM generate_series(1,10) g(n))) <> 10 THEN
        RAISE EXCEPTION 'V50 recommendation feedback refresh failed: expected 10 deterministic rows';
    END IF;
    IF (SELECT COUNT(*) FROM cinema_concession_cost_basis WHERE id IN (SELECT md5('seed51:cost-basis:' || n)::uuid FROM generate_series(1,10) g(n))) <> 10 THEN
        RAISE EXCEPTION 'V51 concession cost-basis refresh failed: expected 10 deterministic rows';
    END IF;
    IF (SELECT COUNT(*) FROM analytics_snapshot WHERE id IN (SELECT md5('seed51:analytics-snapshot:' || n)::uuid FROM generate_series(1,10) g(n))) <> 10 THEN
        RAISE EXCEPTION 'V51 analytics snapshot refresh failed: expected 10 deterministic rows';
    END IF;
