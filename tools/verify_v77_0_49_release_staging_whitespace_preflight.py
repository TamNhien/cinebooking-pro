...
ok(any(x in sw for x in ['const VERSION = "v77-0-49";','const VERSION = "v77-0-50";','const VERSION = "v77-0-52";','const VERSION = "v77-0-53";']), 'Service Worker release metadata is V77.0.49 or forward-compatible V77.0.50')
...
ok(any(x in readme for x in ['Current release:** V77.0.49','Current release:** V77.0.50','Current release:** V77.0.52','Current release:** V77.0.53']) and any(x in readme for x in ['`v77.0.49`','`v77.0.50`','`v77.0.52`','`v77.0.53`']), 'README retains V77.0.49 history under the V77.0.50-or-newer stable target')
ok([p.name for p in ROOT.glob('*.md')]==['README.md'], 'Source keeps one consolidated root Markdown history document')
ok(any(x in prev for x in ['v77-0-49','v77-0-50','v77-0-52','v77-0-53']) and 'V77.0.49' in prev, 'V77.0.48 verifier remains forward-compatible through V77.0.50')
