...
check('Redis hold service reads authoritative TTL', 'getExpire(redisKey, TimeUnit.SECONDS)' in holds or ('POSTGRESQL_WITH_REDIS_MIRROR' in holds and 'remainingSeconds' in holds and 'findActiveForSeats' in holds))
check('Redis hold TTL uses minimum across multi-seat hold', 'Math.min(remaining, ttl)' in holds or ('mapToLong' in holds and '.min().orElse(0L)' in holds and 'SeatHoldRecord::getExpiresAt' in holds))
check('Redis hold TTL verifies current user ownership', 'userId.toString().equals(owner)' in holds or ('filter(x->userId.equals(x.getUserId()))' in holds and 'remainingMillis' in holds))
check('Atomic Redis multi-seat acquire remains intact', ("redis.call('set', key, ARGV[1], 'PX', ARGV[2])" in holds and 'return 0' in holds) or ('POSTGRESQL_WITH_REDIS_MIRROR' in holds and 'findByIdInForUpdate' in text('backend/src/main/java/com/cinebooking/seat/SeatRepository.java') and 'uq_seat_hold_active' in text('backend/src/main/resources/db/migration/V66__durable_seat_holds.sql')))
check('Atomic Redis owner-only release remains intact', "redis.call('get', key) == ARGV[1]" in holds or ('active.stream().filter(x->userId.equals(x.getUserId())).toList()' in holds and 'row.setState("RELEASED")' in holds))
...
check('Standalone RC defaults to V39-or-newer candidate', bool(rc_version) and int(rc_version.group(1))>=39 and re.search(r'cinebooking_v(?:39|[4-9]\d)_rc_\$\{\{ github\.run_id \}\}',rc) is not None)
check('Standalone RC keeps V39 browser coverage', 'V38 + V39' in rc)
stable_version=re.search(r'default: "(\d+)\.\d+\.\d+"',release)
check('Stable release defaults to V39 or newer', bool(stable_version) and int(stable_version.group(1))>=39 and re.search(r'cinebooking_v(?:39|[4-9]\d)_release_\$\{\{ github\.run_id \}\}',release) is not None)
