...
sql_path = root / 'tools' / 'seed-demo-52-tables-10-rows.sql'
ps1_path = root / 'tools' / 'seed-demo-52-tables.ps1'
check_sql = root / 'tools' / 'check-demo-52-table-counts.sql'

# V48 has 51 application tables plus Flyway metadata = 52 tables in pgAdmin.
...
all_52_tables = expected_seed_inserts | {'movie','flyway_schema_history'}
canonical_movie_ids = [
...
verification_array = set(re.findall(r"'([a-z_]+)'", sql[sql.find('Quick verification of all 52 tables'):]))
main_data = sql[:sql.find('-- Fail if seeded reference rows')]

check('V48 52-table SQL exists and decodes as UTF-8', sql_path.exists() and 'Nguyễn Minh An' in sql and 'Bắp Phô Mai Lớn' in sql)
check('V48 52-table PowerShell runner exists', ps1_path.exists())
check('All 50 seeded application tables have INSERT coverage', expected_seed_inserts.issubset(targets) and len(expected_seed_inserts) == 50)
...
check('Final verification enumerates all 52 pgAdmin tables', all_52_tables.issubset(verification_array) and len(all_52_tables) == 52)
check('Row-count helper includes V46/V47/V48 tables', counts.count("'trusted_device'") == 2 and counts.count("'security_alert'") == 2 and counts.count("'payment_event'") == 2 and counts.count("'cinema_concession_inventory'") == 2 and counts.count("'cinema_concession_price'") == 2)
...
print(f"\nSeed V48 52-table realistic-data verification: {passed}/{len(checks)} checks passed")
