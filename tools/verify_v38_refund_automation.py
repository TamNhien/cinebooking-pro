...
check('V38 Playwright verifies automatic REFUNDED booking state', 'Trạng thái booking: REFUNDED' in e2e or ('booking-status' in e2e and 'data-booking-status="REFUNDED"' in e2e))
check('V38 Playwright verifies refunded payment state', 'paymentCard.getByText("REFUNDED", { exact: true })' in e2e or ('payment-history-item' in e2e and 'data-payment-status="REFUNDED"' in e2e))
