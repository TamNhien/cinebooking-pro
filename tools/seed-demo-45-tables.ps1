...
Write-Host "=== CineBooking: repair/seed DEMO45 data across pgAdmin tables ===" -ForegroundColor Cyan
...
Write-Host "flyway_schema_history is intentionally left untouched." -ForegroundColor Yellow
...
$sql = Join-Path $PSScriptRoot 'seed-demo-45-tables-10-rows.sql'
if (-not (Test-Path $sql)) {
    throw "Seed SQL not found: $sql"
}

$remoteSql = '/tmp/cinebooking-seed-demo45-utf8.sql'
try {
...
Write-Host "Synthetic Phim Demo rows were removed; seeded relations now use the 8 canonical movies." -ForegroundColor Green
