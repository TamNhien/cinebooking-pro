...
check('step-up protected group count raised to nine','PROTECTED_ACTION_GROUPS=9' in stepup_service)
...
check('V69 E2E verifies version order','sort((a,b)=>a-b)' in e2e and 'toBe(69)' in e2e)
...
check('Integration expects Flyway >=69','isGreaterThanOrEqualTo(69)' in itest)
check('Integration expects at least 61 public tables','publicTables).isGreaterThanOrEqualTo(61)' in itest)
...
# CI / release / docs
check('CI source regression names V69','V26-V69 source regression' in ci)
...
check('Release example is v69 stable','such as v69.0.0' in release)
check('README current release is V69','Current release:** V69' in readme or 'Current release: **V69**' in readme)
check('README title is V69','# CineBooking Pro V69' in readme)
...
check('README preserves real-data policy','V52/V65/V66/V67/V68/V69 **không tạo phim/khách/booking/payment giả**' in readme)
