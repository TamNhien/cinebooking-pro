...
check('step-up protected group count raised to nine or later',any(x in stepup_service for x in ['PROTECTED_ACTION_GROUPS=9','PROTECTED_ACTION_GROUPS=10','PROTECTED_ACTION_GROUPS=11']))
...
check('Integration expects Flyway >=69',any(x in itest for x in ['isGreaterThanOrEqualTo(69)','isGreaterThanOrEqualTo(70)','isGreaterThanOrEqualTo(71)']))
check('Integration expects at least 61 public tables',any(x in itest for x in ['publicTables).isGreaterThanOrEqualTo(61)','publicTables).isGreaterThanOrEqualTo(63)','publicTables).isGreaterThanOrEqualTo(65)']))
...
# CI / release / docs
check('CI source regression names V69 or later',any(x in ci for x in ['V26-V69 source regression','V26-V70 source regression','V26-V71 source regression']))
...
check('Release example is stable-only current version',any(x in release for x in ['such as v69.0.0','such as v70.0.0','such as v71.0.0']))
check('README current release is V69 or later',any(x in readme for x in ['Current release:** V69','Current release: **V69**','Current release:** V70','Current release: **V70**','Current release:** V71','Current release: **V71**']))
check('README title is V69 or later',any(x in readme for x in ['# CineBooking Pro V69','# CineBooking Pro V70','# CineBooking Pro V71']))
...
check('README preserves real-data policy',any(x in readme for x in ['V52/V65/V66/V67/V68/V69 **không tạo phim/khách/booking/payment giả**','V52/V65/V66/V67/V68/V69/V70 **không tạo phim/khách/booking/payment giả**','V52/V65/V66/V67/V68/V69/V70/V71 **không tạo phim/khách/booking/payment giả**']))
