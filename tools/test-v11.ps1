...
Write-Host "CineBooking V11 API smoke test" -ForegroundColor Cyan
...
try {
  ...
  # V11 regression: a CANCELLED shift must no longer reserve its old slot.
  Invoke-RestMethod -Method Post -Uri "$BaseUrl/admin/shifts/$($Shift.id)/cancel" -Headers $AdminHeaders | Out-Null
  Write-Pass "Cancel shift"
  $Shift = Invoke-RestMethod -Method Post -Uri "$BaseUrl/admin/shifts" -Headers $AdminHeaders -ContentType "application/json" -Body (To-Json $ShiftBody)
  Write-Pass "Recreate same slot after cancellation (V11 fix)"
  ...
  Write-Host "ALL V11 SMOKE TESTS PASSED" -ForegroundColor Green
}
catch {
