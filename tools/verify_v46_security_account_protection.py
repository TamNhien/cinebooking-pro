...
ok("V46 Playwright covers Brave trust-device and admin alert journey",has("frontend/e2e/security-account-protection.spec.ts","V46 user trusts a Brave device","navigator,\"brave\"","Laptop E2E V46","Security Operations","toContainText(\"Brave\")","logoutToLogin","page.waitForURL","context.clearCookies","/api/auth/logout","expect(response.status()).toBe(204)","page.goto(\"/login\"","const auth=await response.json()","context.request.get","currentNewDeviceAlert","/api/me/security/alerts","/api/admin/security/alerts","customerNewDeviceAlert.id","item.userEmail===email"))
ok("V46 Playwright avoids page.evaluate auth races during navigation",not has("frontend/e2e/security-account-protection.spec.ts","page.evaluate("))
