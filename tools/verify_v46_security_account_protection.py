...
ok("V46 Playwright covers Brave trust-device and admin alert journey",has("frontend/e2e/security-account-protection.spec.ts","V46 user trusts a Brave device","navigator,\"brave\"","Laptop cá nhân","Security Operations","toContainText(\"Brave\")","logoutToLogin","context.clearCookies","/api/auth/logout","expect(status).toBe(204)","page.goto(\"/login\"","authFromStorage","context.storageState()","context.request.get","currentNewDeviceAlert","/api/me/security/alerts","/api/admin/security/alerts","customerNewDeviceAlert.id","item.userEmail===email"))
