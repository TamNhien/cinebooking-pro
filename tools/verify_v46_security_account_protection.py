...
ok("Admin dashboard links Security Operations",has("frontend/app/admin/page.tsx","/admin/security") and (has("frontend/app/admin/page.tsx","Security Operations") or has("frontend/app/admin/page.tsx","Security & Identity V68")))
ok("V46 Playwright covers Brave trust-device and admin alert journey",has("frontend/e2e/security-account-protection.spec.ts","V46 user trusts a Brave device","navigator,\"brave\"","Laptop cá nhân","Vận hành bảo mật · Bảo mật & định danh","toContainText(\"Brave\")","logoutToLogin","context.clearCookies","/api/auth/logout","expect(status).toBe(204)","page.goto(\"/login\"","authFromStorage","context.storageState()","context.request.get","currentNewDeviceAlert","/api/me/security/alerts","/api/admin/security/alerts","customerNewDeviceAlert.id","item.userEmail===email"))
ok("V46 Playwright logout helper avoids competing header and login navigations",has("frontend/e2e/security-account-protection.spec.ts","page.evaluate(async(authStorageKey)","fetch(\"/api/auth/logout\"","localStorage.removeItem(authStorageKey)","return response.status","expect(status).toBe(204)","context.clearCookies()","page.goto(\"/login\"") and not has("frontend/e2e/security-account-protection.spec.ts","logoutResponse=page.waitForResponse","getByRole(\"button\",{name:\"Đăng xuất\"}).click()","page.waitForURL(url=>url.pathname===\"/\""))
ok("V46 Playwright avoids browser-response-body auth races during navigation",not has("frontend/e2e/security-account-protection.spec.ts","const auth=await response.json()") and has("frontend/e2e/security-account-protection.spec.ts","context.storageState()","AUTH_STORAGE_KEY"))
