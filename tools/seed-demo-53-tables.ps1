...
Write-Host "=== CineBooking V49: realistic reference data across all 53 pgAdmin tables ===" -ForegroundColor Cyan
...
Write-Host "V49 smart showtime planning: 10 committed planning runs are seeded; V48 branch stock/pricing and V47 payment history remain covered." -ForegroundColor Cyan
...
$sql = Join-Path $PSScriptRoot 'seed-demo-53-tables-10-rows.sql'
if (-not (Test-Path $sql)) {
    throw "Seed SQL not found: $sql"
}

$remoteSql = '/tmp/cinebooking-reference53-utf8.sql'
try {
...
Write-Host "All 53 pgAdmin tables now have data; Flyway history remains genuine." -ForegroundColor Green
...
Write-Host "security_alert: 10 risk-scored security alerts.
Write-Host "showtime_planning_run: 10 realistic smart-planner audit runs." -ForegroundColor Green" -ForegroundColor Green
