...
Set-Location $root
$sql = Join-Path $PSScriptRoot 'check-demo-53-table-counts.sql'
$remoteSql = '/tmp/cinebooking-check-53-tables.sql'
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Could not inspect 53 table row counts." }
}
finally {
