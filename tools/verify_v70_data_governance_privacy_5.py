...
check('Protected action groups raised to ten or later',any(x in stepup_service for x in ['PROTECTED_ACTION_GROUPS=10','PROTECTED_ACTION_GROUPS=11']))
...
check('Integration expects Flyway >=70',any(x in itest for x in ['isGreaterThanOrEqualTo(70)','isGreaterThanOrEqualTo(71)']))
check('Integration expects at least 63 public tables',any(x in itest for x in ['publicTables).isGreaterThanOrEqualTo(63)','publicTables).isGreaterThanOrEqualTo(65)']))
...
check('V69 verifier forward-compatible with V70',any(x in v69verify for x in ['isGreaterThanOrEqualTo(70)','isGreaterThanOrEqualTo(71)']) and any(x in v69verify for x in ['63','65']))
...
check('CI source regression names V70 or later',any(x in ci for x in ['V26-V70 source regression','V26-V71 source regression']))
...
check('Release example is V70 or later stable',any(x in release for x in ['such as v70.0.0','such as v71.0.0']))
check('Release remains stable-only','Pre-release tags are disabled' in release and '-rc.' not in release)
check('README current release V70 or later',any(x in readme for x in ['Current release:** V70','Current release: **V70**','Current release:** V71','Current release: **V71**']))
check('README title V70 or later',any(x in readme for x in ['# CineBooking Pro V70','# CineBooking Pro V71']))
...
check('README preserves real-data policy',any(x in readme for x in ['V52/V65/V66/V67/V68/V69/V70 **không tạo phim/khách/booking/payment giả**','V52/V65/V66/V67/V68/V69/V70/V71 **không tạo phim/khách/booking/payment giả**']))
