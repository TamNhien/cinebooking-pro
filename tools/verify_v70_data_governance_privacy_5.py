...
check('Protected action groups raised to ten','PROTECTED_ACTION_GROUPS=10' in stepup_service)
...
check('Integration expects Flyway >=70','isGreaterThanOrEqualTo(70)' in itest)
check('Integration expects at least 63 public tables','publicTables).isGreaterThanOrEqualTo(63)' in itest)
...
check('V69 verifier forward-compatible with V70',('isGreaterThanOrEqualTo(70)' in v69verify or "'isGreaterThanOrEqualTo(70)'" in v69verify) and ('63' in v69verify))
check('V68 verifier forward-compatible with V70','70' in v68verify and '63' in v68verify)
check('V70 adds no synthetic seeded business rows','V70' not in seed)

# CI / tooling / release / docs
check('CI source regression names V70','V26-V70 source regression' in ci)
...
check('Release example is V70 stable','such as v70.0.0' in release)
check('Release remains stable-only','Pre-release tags are disabled' in release and '-rc.' not in release)
check('README current release V70','Current release:** V70' in readme or 'Current release: **V70**' in readme)
check('README title V70','# CineBooking Pro V70' in readme)
...
check('README preserves real-data policy','V52/V65/V66/V67/V68/V69/V70 **không tạo phim/khách/booking/payment giả**' in readme)
