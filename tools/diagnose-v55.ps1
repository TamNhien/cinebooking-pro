...
Write-Host '=== CineBooking V55 source diagnostics ===' -ForegroundColor Cyan
$checks=@(
...
foreach($check in $checks){
  ...
  python $check
  if($LASTEXITCODE -ne 0){throw "V55 diagnostic failed: $check"}
}
Write-Host "`nV55 source diagnostics passed." -ForegroundColor Green
