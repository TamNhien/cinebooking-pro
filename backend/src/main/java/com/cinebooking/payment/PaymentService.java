...
public class PaymentService {
    ...
    public Map<String,Object> momoIpn(Map<String,Object> params){
        ...
        if(!signatureValid){String rejectedKey=rejectedWebhookKey("signature",payloadHash);if(!claimWebhook("MOMO",rejectedKey,p,payloadHash,false,code))return previousMomoResponse(rejectedKey);if(p!=null)events.record(p.getId(),"WEBHOOK_REJECTED","GATEWAY","MOMO",p.getStatus().name(),p.getStatus().name(),"INVALID_SIGNATURE","MoMo IPN bị từ chối do chữ ký không hợp lệ",json("payloadHash",payloadHash));return finishMomo(rejectedKey,p,97,"Invalid signature");}
        if(p==null){if(!claimWebhook("MOMO",eventKey,null,payloadHash,true,code))return previousMomoResponse(eventKey);return finishMomo(eventKey,null,1,"Order not found");}
        if(!validMomoAmount(p,params)){String rejectedKey=rejectedWebhookKey("amount",payloadHash);if(!claimWebhook("MOMO",rejectedKey,p,payloadHash,true,code))return previousMomoResponse(rejectedKey);events.record(p.getId(),"WEBHOOK_REJECTED","GATEWAY","MOMO",p.getStatus().name(),p.getStatus().name(),"INVALID_AMOUNT","MoMo IPN bị từ chối do amount mismatch",json("payloadHash",payloadHash));return finishMomo(rejectedKey,p,1,"Invalid amount");}
    ...
    public PaymentReturnResponse vnPayReturn(Map<String,String> params){boolean signatureValid=vnPay.verify(params);String orderId=params.getOrDefault("vnp_TxnRef","");Payment p=payments.findByProviderInAndProviderOrderId(List.of("VNPAY","VNPAY_QR"),orderId).orElse(null);String message=!signatureValid?"Invalid checksum":p==null?"Order not found":!validVnPayAmount(p,params)?"Invalid amount":"Return verified; waiting for server IPN is allowed";return returnView("VNPAY",p,signatureValid&&p!=null&&validVnPayAmount(p,params),params.getOrDefault("vnp_ResponseCode",""),message);}
    public PaymentReturnResponse momoReturn(Map<String,Object> params){boolean signatureValid=momo.verifyIpn(params);String orderId=String.valueOf(params.getOrDefault("orderId",""));Payment p=payments.findByProviderInAndProviderOrderId(List.of("MOMO","MOMO_QR"),orderId).orElse(null);boolean valid=signatureValid&&p!=null&&validMomoAmount(p,params);String message=!signatureValid?"Invalid signature":p==null?"Order not found":!validMomoAmount(p,params)?"Invalid amount":"Return verified; payment state comes from server IPN/reconciliation";return returnView("MOMO",p,valid,String.valueOf(params.getOrDefault("resultCode","")),message);}
