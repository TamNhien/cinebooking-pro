...
public class AdminPaymentService {
    ...
    public ReconciliationResult reconcile(UUID paymentId,String actor,String ip,String trigger){
        ...
        try{
            ...
            String actorType=trigger!=null&&trigger.toUpperCase(Locale.ROOT).startsWith("AUTO")?"SYSTEM":"ADMIN";events.record(paymentId,"PAYMENT_RECONCILED",actorType,actor,before.name(),current.getStatus().name(),providerStatus,message,json("trigger",trigger));audit.record(actor,"PAYMENT_RECONCILE","PAYMENT",paymentId.toString(),provider+" trigger="+trigger+" providerStatus="+providerStatus+" localStatus="+current.getStatus(),ip);
            ...
            String actorType=trigger!=null&&trigger.toUpperCase(Locale.ROOT).startsWith("AUTO")?"SYSTEM":"ADMIN";events.record(paymentId,"PAYMENT_RECONCILE_FAILED",actorType,actor,before.name(),p.getStatus().name(),"QUERY_FAILED",error,json("trigger",trigger));audit.record(actor,"PAYMENT_RECONCILE_FAILED","PAYMENT",paymentId.toString(),provider+" trigger="+trigger+" error="+error,ip);
    ...
    private WebhookAdminView view(PaymentWebhookEvent e){return new WebhookAdminView(e.getId(),e.getProvider(),e.getEventKey(),e.getPaymentId(),e.getPayloadHash(),e.isSignatureValid(),e.getResultCode(),e.getResponseCode(),e.getResponseMessage(),e.getReceivedAt(),e.getProcessedAt(),e.getDeliveryState(),n(e.getRecoveryAttempts(),0),e.getLastRecoveryAt(),e.getRecoveredAt(),e.getRecoveryMessage());}
