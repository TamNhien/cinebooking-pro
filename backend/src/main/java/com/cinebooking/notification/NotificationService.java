...
public class NotificationService {
    ...
    public boolean createOnce(UUID userId,String type,String title,String message,String link,String dedupeKey){
        ...
        NotificationPreference pref=preference(userId); String category=category(type); String priority=priority(type);
    ...
    public List<NotificationResponse> list(String email,String view){
        UUID uid=user(email); boolean archived="ARCHIVED".equalsIgnoreCase(view);
        return (archived?repo.findTop100ByUserIdAndInAppVisibleTrueAndArchivedAtIsNotNullOrderByArchivedAtDesc(uid):repo.findTop100ByUserIdAndInAppVisibleTrueAndArchivedAtIsNullOrderByCreatedAtDesc(uid)).stream().map(this::dto).toList();
    }
    public NotificationSummary summary(String email){
        UUID uid=user(email);
        return new NotificationSummary(repo.countByUserIdAndInAppVisibleTrueAndArchivedAtIsNullAndReadFalse(uid),repo.countByUserIdAndInAppVisibleTrueAndArchivedAtIsNullAndReadFalseAndPriority(uid,"HIGH"),repo.countByUserIdAndInAppVisibleTrueAndArchivedAtIsNotNull(uid));
    }
    ...
    public List<NotificationResponse> browserFeed(String email,Instant after){
        ...
        return repo.findTop20ByUserIdAndArchivedAtIsNullAndCreatedAtAfterOrderByCreatedAtAsc(uid,safeAfter).stream().map(this::dto).toList();
    }
    ...
    public PreferenceResponse updatePreference(String email,PreferenceUpdate req){
        ...
        if(req.loyaltyEnabled()!=null)p.setLoyaltyEnabled(req.loyaltyEnabled());
        if(req.waitlistEnabled()!=null)p.setWaitlistEnabled(req.waitlistEnabled());
        return prefDto(preferences.save(p));
    }

    @Transactional public NotificationResponse read(UUID id,String email){
        UUID uid=user(email);UserNotification n=owned(id,uid);if(!Boolean.TRUE.equals(n.getRead())){n.setRead(true);n.setReadAt(Instant.now());}return dto(repo.save(n));
    }
    ...
    private UserNotification owned(UUID id,UUID uid){return repo.findByIdAndUserId(id,uid).orElseThrow(()->new ApiException(HttpStatus.NOT_FOUND,"Không tìm thấy thông báo"));}
    ...
    private boolean enabled(NotificationPreference p,String category){return switch(category){case "BOOKING"->Boolean.TRUE.equals(p.getBookingEnabled());case "REMINDER"->Boolean.TRUE.equals(p.getReminderEnabled());case "REFUND"->Boolean.TRUE.equals(p.getRefundEnabled());case "STAFF_SHIFT"->Boolean.TRUE.equals(p.getStaffShiftEnabled());case "PROMOTION"->Boolean.TRUE.equals(p.getPromotionEnabled());case "LOYALTY"->Boolean.TRUE.equals(p.getLoyaltyEnabled());case "WAITLIST"->Boolean.TRUE.equals(p.getWaitlistEnabled());default->true;};}
    private String category(String type){String t=type==null?"":type.toUpperCase(Locale.ROOT);if(t.startsWith("REFUND"))return "REFUND";if(t.startsWith("SHOWTIME_REMINDER"))return "REMINDER";if(t.startsWith("STAFF_SHIFT"))return "STAFF_SHIFT";if(t.startsWith("PROMOTION")||t.startsWith("VOUCHER"))return "PROMOTION";if(t.startsWith("LOYALTY")||t.startsWith("BIRTHDAY"))return "LOYALTY";if(t.startsWith("WAITLIST"))return "WAITLIST";if(t.startsWith("PAYMENT")||t.startsWith("BOOKING"))return "BOOKING";return "GENERAL";}
    private String priority(String type){String t=type==null?"":type.toUpperCase(Locale.ROOT);if(t.startsWith("SECURITY_ALERT")||t.startsWith("WAITLIST")||t.startsWith("REFUND_REVIEW")||t.startsWith("PAYMENT_REVIEW")||t.startsWith("SHOWTIME_REMINDER_30M"))return "HIGH";if(t.startsWith("PROMOTION"))return "LOW";return "NORMAL";}
    private NotificationResponse dto(UserNotification n){return new NotificationResponse(n.getId(),n.getNotificationType(),n.getCategory(),n.getPriority(),n.getTitle(),n.getMessage(),n.getLinkUrl(),Boolean.TRUE.equals(n.getRead()),n.getReadAt(),n.getArchivedAt()!=null,n.getArchivedAt(),n.getEmailStatus(),n.getCreatedAt());}
    private PreferenceResponse prefDto(NotificationPreference p){return new PreferenceResponse(Boolean.TRUE.equals(p.getInAppEnabled()),Boolean.TRUE.equals(p.getEmailEnabled()),Boolean.TRUE.equals(p.getBrowserEnabled()),Boolean.TRUE.equals(p.getBookingEnabled()),Boolean.TRUE.equals(p.getReminderEnabled()),Boolean.TRUE.equals(p.getRefundEnabled()),Boolean.TRUE.equals(p.getStaffShiftEnabled()),Boolean.TRUE.equals(p.getPromotionEnabled()),Boolean.TRUE.equals(p.getLoyaltyEnabled()),Boolean.TRUE.equals(p.getWaitlistEnabled()),p.getUpdatedAt());}
