...
public class SecurityConfig {
    ...
    @Bean CorsConfigurationSource cors(@Value("${app.frontend-url}") String frontendUrl) {
        ...
        c.setAllowedHeaders(List.of("Authorization", "Content-Type", "Idempotency-Key", "X-CineBooking-Browser", "Sec-CH-UA", "X-Trace-Id", "X-Step-Up-Token"));
    ...
    @Bean SecurityFilterChain securityFilterChain(HttpSecurity http, JwtAuthenticationFilter jwt, StepUpAuthorizationFilter stepUp, SecurityHeadersFilter securityHeaders) throws Exception {
        http
            ...
            .authorizeHttpRequests(auth -> auth
                ...
                .requestMatchers("/api/admin/operations-control/**").hasAnyRole("MANAGER","ADMIN")
                ...
                .requestMatchers("/api/admin/risk/**").hasRole("ADMIN")
