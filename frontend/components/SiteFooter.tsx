...
export default function SiteFooter(){
  ...
  return <footer className="site-footer">
    <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 md:grid-cols-4 md:px-6">
      ...
      <div><h3>{en?"Customer":"Khách hàng"}</h3><Link href="/bookings">{en?"My tickets":"Vé của tôi"}</Link><Link href="/waitlist">{en?"Seat alerts":"Chờ ghế trống"}</Link><Link href="/profile">{en?"Account":"Tài khoản"}</Link><Link href="/forgot-password">{en?"Forgot password":"Quên mật khẩu"}</Link></div>
      <div><h3>{en?"Information":"Thông tin"}</h3><p>{en?"Demo hotline: 1900 0000":"Đường dây nóng thử nghiệm: 1900 0000"}</p><p>{en?"Email: support@cinebooking.local":"Thư điện tử: support@cinebooking.local"}</p><p>{en?"Support 08:00–22:00":"Hỗ trợ 08:00–22:00"}</p></div>
    </div>
