...
export default function QuickBooking(){
  ...
  return <section className="quick-booking" aria-label={en?"Quick booking":"Đặt vé nhanh"}>
    ...
    <div className="quick-booking-grid">
      <label><span>{en?"1. Movie":"1. Phim"}</span><select className="input" value={movieId} onChange={e=>setMovieId(e.target.value)}><option value="">{en?"Select movie":"Chọn phim"}</option>{movies.map(m=><option value={m.id} key={m.id} data-testid="quick-booking-movie-option-v7815" data-i18n-skip="true">{m.title}</option>)}</select></label>
      <label><span>{en?"2. Cinema":"2. Rạp"}</span><select className="input" value={cinemaId} onChange={e=>setCinemaId(e.target.value)} disabled={!movieId}><option value="">{en?"Select cinema":"Chọn rạp"}</option>{cinemas.map(c=><option value={c.id} key={c.id} data-testid="quick-booking-cinema-option-v7815" data-i18n-skip="true">{c.name}</option>)}</select></label>
