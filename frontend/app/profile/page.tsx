    ...
    <div className="profile-page mx-auto max-w-5xl space-y-6">
      ...
      <section className="profile-member-grid grid gap-4 sm:grid-cols-3">
        <div className="card p-5 sm:col-span-2">
          <div className="text-xs font-bold tracking-[.2em] text-rose-400">THÀNH VIÊN CINEBOOKING · V40</div>
          ...
          <div className="mt-2 flex flex-wrap justify-between gap-2 text-xs text-slate-500"><span>Trọn đời: <span data-testid="loyalty-lifetime-points">{member?.lifetimePoints ?? profile.loyaltyLifetimePoints}</span> điểm</span><span>{member?.nextTier ? `Còn ${member.pointsToNextTier} điểm → ${member.nextTier}` : "Đã đạt hạng cao nhất"}</span></div>
          <div className="mt-3 text-xs text-slate-500">ĐỒNG · 500 BẠC · 1.500 VÀNG · 4.000 KIM CƯƠNG</div>
        </div>
      ...
      <section className="grid gap-4 md:grid-cols-2">
        <div className="card p-5">
          ...
          <div className="mt-3 text-sm text-slate-300">{member?.nextExpiryAt ? `Lô gần nhất hết hạn: ${new Date(member.nextExpiryAt).toLocaleString(presentationLocale())}` : "Hiện chưa có lô điểm đang chờ hết hạn."}</div>
        </div>
        <div className="card p-5">
          ...
          {!profile.birthDate ? <p className="mt-3 text-sm text-amber-300">Hãy thiết lập ngày sinh bên dưới để kích hoạt quyền lợi.</p> : <p className="mt-3 text-sm text-slate-300">Ngày sinh: {new Date(`${profile.birthDate}T00:00:00`).toLocaleDateString(presentationLocale())}</p>}
      ...
      <section className="card profile-card p-6">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{rewards.map(r => <article key={r.id} data-testid={`loyalty-reward-${r.code.toLowerCase()}`} className="rounded-2xl border border-slate-800 bg-slate-950/45 p-4"><div className="text-xs font-black text-rose-300">{r.rewardType === "CONCESSION" ? "BẮP NƯỚC" : "VOUCHER"}</div><h3 className="mt-2 font-bold" data-i18n-skip={isSystemLoyaltyPresentation(r.name) ? undefined : "true"}>{systemLoyaltyPresentation(r.name, language)}</h3><p className="mt-1 min-h-10 text-xs leading-5 text-slate-500" data-i18n-skip={isSystemLoyaltyPresentation(r.description) ? undefined : "true"}>{systemLoyaltyPresentation(r.description, language)}</p><div className="mt-3 flex items-end justify-between gap-3"><div><div className="text-xl font-black text-amber-300">{r.pointsCost} điểm</div><div className="text-xs text-slate-500">Hiệu lực {r.validityDays} ngày</div></div><button type="button" data-testid={`loyalty-redeem-${r.code.toLowerCase()}`} disabled={busy || !r.canRedeem} onClick={()=>redeemReward(r.id)} className="btn btn-primary !px-3 !py-2">{r.canRedeem ? "Đổi" : "Chưa đủ"}</button></div></article>)}{!rewards.length&&<div className="text-sm text-slate-500">Chưa có phần thưởng đang mở.</div>}</div>
      </section>

      <section className="card profile-card p-6">
        <h2 className="text-xl font-bold">🎟 Ví phần thưởng</h2>
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          <div><h3 className="font-bold">Mã ưu đãi cá nhân</h3><div className="mt-2 space-y-2">{ownedVouchers.map(v=><div key={v.id} className="rounded-xl border border-slate-800 p-3"><div className="font-mono font-black text-amber-300">{v.code}</div><div className="mt-1 text-sm" data-i18n-skip={isSystemLoyaltyPresentation(v.name) ? undefined : "true"}>{systemLoyaltyPresentation(v.name, language)}</div><div className="mt-1 text-xs text-slate-500">{v.active?"Có thể sử dụng":"Đã dùng / hết hạn"}{v.endsAt?` · hết hạn ${new Date(v.endsAt).toLocaleDateString(presentationLocale())}`:""}</div></div>)}{!ownedVouchers.length&&<div className="text-sm text-slate-500">Chưa có mã ưu đãi cá nhân.</div>}</div></div>
          <div><h3 className="font-bold">Mã nhận bắp nước</h3><div className="mt-2 space-y-2">{redemptions.filter(r=>r.rewardType==="CONCESSION").map(r=><div key={r.id} className="rounded-xl border border-slate-800 p-3"><div className="font-mono font-black text-emerald-300">{r.redemptionCode}</div><div className="mt-1 text-sm" data-i18n-skip={isSystemLoyaltyPresentation(r.rewardName) ? undefined : "true"}>{systemLoyaltyPresentation(r.rewardName, language)}</div><div className="mt-1 text-xs text-slate-500">{r.status==="CLAIMED"?`Đã nhận ${r.claimedAt?new Date(r.claimedAt).toLocaleString(presentationLocale()):""}`:`Chưa nhận · hết hạn ${new Date(r.expiresAt).toLocaleDateString(presentationLocale())}`}</div></div>)}{!redemptions.some(r=>r.rewardType==="CONCESSION")&&<div className="text-sm text-slate-500">Chưa có mã nhận quà tại quầy.</div>}</div></div>
        </div>
      </section>
        ...
        <div className="space-y-3">
          {sessions.map(s => <div key={s.id} className={`rounded-2xl border p-4 ${s.current ? "border-emerald-500/40 bg-emerald-500/5" : "border-slate-800 bg-slate-950/40"}`}>
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div><div className="font-bold">{s.deviceName} {s.current && <span className="ml-2 rounded-full bg-emerald-500/15 px-2 py-1 text-[10px] font-black text-emerald-300">PHIÊN HIỆN TẠI</span>}</div><div className="mt-1 text-xs text-slate-500">IP {s.ipAddress || "—"} · xác thực {new Date(s.lastSeenAt).toLocaleString(presentationLocale())}</div><div className="mt-1 text-xs text-slate-600">Hết hạn refresh: {new Date(s.expiresAt).toLocaleString(presentationLocale())}</div>{!s.active&&<div className="mt-2 text-xs font-semibold text-rose-300">Đã thu hồi{s.revokeReason?` · ${s.revokeReason}`:""}</div>}</div>
