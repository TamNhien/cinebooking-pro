...
export default function BookingPage({params}:{params:Promise<{showtimeId:string}>}){
  ...
  return <div className="grid gap-7 lg:grid-cols-[minmax(0,1fr)_380px]">
    <section className="space-y-6 min-w-0">
      <div className="card p-5 md:p-7">
        ...
        {rows.length===0?<div className="mx-auto max-w-xl rounded-2xl border border-amber-700/50 bg-amber-950/25 p-6 text-center"><div className="text-4xl">💺</div><h2 className="mt-3 text-lg font-bold text-amber-100">{en?"No seats configured for this auditorium":"Phòng chiếu này chưa có sơ đồ ghế"}</h2><p className="mt-2 text-sm leading-6 text-amber-200/80">{en?"An administrator needs to generate or add seats before customers can book this showtime.":"Admin cần tạo sơ đồ ghế cho phòng chiếu trước khi khách đặt vé."}</p>{auth?.role==="ADMIN"&&<Link href="/admin" className="btn btn-primary mt-4 inline-flex">{en?"Open Admin":"Mở trang Admin"}</Link>}</div>:<>
          <div aria-label={en?"Smart seat recommendation":"Gợi ý ghế thông minh"} className="mb-6 rounded-2xl border border-violet-800/50 bg-violet-950/20 p-4"><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><div className="font-bold text-violet-100">✨ {en?"Best contiguous seats":"Gợi ý cụm ghế đẹp"}</div><p className="mt-1 text-xs leading-5 text-slate-400">{en?"Ranks available contiguous seats near the row center and avoids creating a single-seat gap.":"Ưu tiên ghế liền nhau gần trung tâm và tránh tạo ghế trống đơn lẻ."}</p></div><div className="flex flex-wrap items-center gap-2"><label className="text-xs text-slate-400" htmlFor="party-size">{en?"Party":"Số người"}</label><select id="party-size" aria-label={en?"Party size":"Số người cần xếp ghế"} className="input !w-24 !py-2" value={partySize} disabled={held} onChange={e=>setPartySize(Number(e.target.value))}>{Array.from({length:Math.min(8,map.maxSelectableSeats)},(_,i)=>i+1).map(n=><option key={n} value={n}>{n}</option>)}</select><button type="button" className="btn btn-secondary" disabled={held||suggestionBusy} onClick={suggestSeats}>{suggestionBusy?(en?"Finding...":"Đang tìm..."):(en?"Suggest seats":"Gợi ý ghế")}</button></div></div>{suggestions.length>0&&<div className="mt-3 grid gap-2 md:grid-cols-2">{suggestions.slice(0,4).map((suggestion,index)=><button type="button" key={suggestion.seatIds.join("-")} onClick={()=>chooseSuggestion(suggestion.seatIds)} disabled={held} className="rounded-xl border border-slate-700/80 bg-slate-950/45 p-3 text-left transition hover:border-violet-500"><div className="flex items-center justify-between gap-3"><strong>{index===0?"★ ":""}{suggestion.seatCodes.join(", ")}</strong><span className="text-sm text-amber-300">{currency(suggestion.totalPrice)}</span></div><p className="mt-1 text-xs text-slate-400">{suggestion.reason}</p></button>)}</div>}</div>
    ...
    <aside className="card h-fit p-6 lg:sticky lg:top-24">
      ...
      {held&&<div aria-label={en?"Server synchronized seat hold countdown":"Đếm ngược giữ ghế đồng bộ máy chủ"} className={`mt-4 rounded-xl p-3 text-center text-sm ${seconds<=60?"border border-amber-700/60 bg-amber-950/45 text-amber-200":"bg-rose-950/40 text-rose-200"}`}>{en?"Seats held for":"Ghế được giữ trong"} <strong>{Math.floor(seconds/60)}:{String(seconds%60).padStart(2,"0")}</strong><div className="mt-1 text-[11px] opacity-75">{seconds<=60?(en?"Complete checkout soon — less than one minute remains.":"Hãy thanh toán sớm — thời gian giữ ghế còn dưới 1 phút."):(en?"Synced with Redis hold TTL every 15 seconds.":"Đồng bộ TTL giữ ghế Redis mỗi 15 giây.")}</div></div>}
