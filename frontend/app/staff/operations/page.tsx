...
import { staffHandoverSummary, staffIncidentPresentation } from "@/lib/controlled-business-presentation";
...
export default function StaffOperationsPage(){
  ...
  return <div className="mx-auto max-w-7xl space-y-7">
    ...
    <div className="grid gap-6 xl:grid-cols-2">
      ...
      <div className="mt-5 space-y-3">{incidents.map(i=>{const copy=staffIncidentPresentation(i.title,i.description,i.resolutionNote,language);return <article key={i.id} data-testid="staff-incident" className="rounded-xl border border-slate-800 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><b>{localizedLabel(i.severity,language)} · {localizedLabel(i.category,language)} · {copy.title}</b><span className="text-xs">{localizedLabel(i.status,language)}</span></div><p className="mt-2 whitespace-pre-wrap text-sm text-slate-300">{copy.description}</p><div className="mt-2 text-xs text-slate-500">{t("Báo bởi","Reported by")} {i.reportedByName} · {formatDate(i.createdAt)}</div>{i.status==="RESOLVED"&&<div className="mt-2 rounded-lg bg-slate-900 p-3 text-sm">{t("Đã xử lý","Resolved")}: {copy.resolutionNote}</div>}{i.status==="OPEN"&&canResolve&&<div className="mt-3 flex flex-col gap-2 sm:flex-row"><input className="input" placeholder={t("Ghi chú xử lý","Resolution note")} value={resolution[i.id]||""} onChange={e=>setResolution(v=>({...v,[i.id]:e.target.value}))}/><button className="btn btn-secondary shrink-0" onClick={()=>resolveIncident(i.id)}>{t("Đóng sự cố","Close incident")}</button></div>}</article>})}{incidents.length===0&&<p className="text-sm text-slate-500">{t("Chưa có sự cố.","No incidents yet.")}</p>}</div></section>
    </div>
  </div>;
}
