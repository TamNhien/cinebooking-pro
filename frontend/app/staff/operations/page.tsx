...
export default function StaffOperationsPage(){
  ...
  return <div className="mx-auto max-w-7xl space-y-7">
    ...
    <section className="card p-5"><div className="flex items-center justify-between gap-3"><div><h2 className="text-xl font-bold">{t("Lượt soát vé theo thời gian thực","Realtime check-ins")} · {live.cinemaName}</h2><p className="text-xs text-slate-500">{t("WebSocket tự làm mới; dự phòng kiểm tra mỗi 15 giây.","WebSocket refreshes automatically with a 15-second polling fallback.")}</p></div><span className="text-xs text-slate-500">{formatDate(live.generatedAt)}</span></div>
