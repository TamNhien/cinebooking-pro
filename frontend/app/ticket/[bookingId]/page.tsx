...
export default function TicketPage({params}:{params:Promise<{bookingId:string}>}){
 ...
 return <div className="ticket-print-card mx-auto max-w-lg card p-7 text-center">
   <p className="section-kicker">E-TICKET · PWA V52</p><h1 className="text-3xl font-bold">Vé điện tử</h1>
   ...
   {(ticket||offline)&&!checkedIn&&!staleOffline&&<div className="mt-5 rounded-xl border border-slate-700 bg-slate-950/50 p-3 text-left text-xs leading-5 text-slate-400"><b className="text-slate-200">QR CineBooking:</b> camera mặc định trên điện thoại có thể mở thẳng trang check-in của CineBooking.{localOnly&&<div className="mt-2 text-amber-300">⚠ QR đang dùng {publicBase}. Điện thoại khác không truy cập được localhost. Hãy đặt TICKET_PUBLIC_BASE_URL thành IP LAN/domain của máy chủ rồi recreate backend.</div>}</div>}
