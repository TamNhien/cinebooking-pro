...
export default function TicketPage({params}:{params:Promise<{bookingId:string}>}){
 ...
 useEffect(()=>{let active=true;(async()=>{
   try{
     ...
     else if(active)setError(cached&&!auth?.userId?"Hãy đăng nhập lại đúng tài khoản để mở bản vé offline này.":(e as Error).message);
   }
 ...
 return <div className="ticket-print-card mx-auto max-w-lg card p-7 text-center">
   ...
   {usingOffline&&<div className={`mt-4 rounded-xl border p-3 text-sm font-semibold ${staleOffline?"border-red-700/60 bg-red-950/35 text-red-200":"border-cyan-800/50 bg-cyan-950/30 text-cyan-200"}`}>{staleOffline?`⛔ Bản vé offline này đã bị máy chủ đánh dấu không còn hợp lệ: ${offline?.invalidReason||"Hãy kết nối mạng để kiểm tra lại."}`:"📴 Đang hiển thị bản vé offline đã lưu trên thiết bị."}</div>}
   ...
   {qrDataUrl&&!staleOffline&&<img src={qrDataUrl} alt="QR URL vé CineBooking" className={`mx-auto mt-6 w-72 rounded-2xl bg-white p-3 ${checkedIn?"opacity-45":""}`}/>}
   {error&&<p className="mt-5 text-red-300">{error}</p>}
   {!usingOffline&&booking&&ticket&&qrDataUrl&&<div className="mt-5 flex flex-wrap justify-center gap-2"><button type="button" className="btn btn-primary" disabled={saving} onClick={save}>{saving?"Đang lưu...":offline?"↻ Cập nhật vé offline":"⬇ Lưu vé offline"}</button>{booking.status==="CONFIRMED"&&<button type="button" className="btn btn-secondary" disabled={calendarBusy} onClick={addCalendar}>📅 {calendarBusy?"Đang tạo...":"Thêm vào lịch"}</button>}<button type="button" className="btn btn-secondary" onClick={copyCode}>📋 Mã đặt vé</button><button type="button" className="btn btn-secondary" onClick={()=>window.print()}>🖨 In vé</button>{transferEligibility?.allowed&&<button type="button" className="btn btn-secondary" onClick={()=>setTransferOpen(v=>!v)}>🎁 Chuyển/tặng vé</button>}{offline&&<button type="button" className="btn btn-secondary" onClick={remove}>Xóa bản ngoại tuyến</button>}<Link href="/offline-tickets" className="btn btn-secondary">Vé ngoại tuyến</Link></div>}
   ...
   {(ticket||offline)&&!checkedIn&&!staleOffline&&<div className="mt-5 rounded-xl border border-slate-700 bg-slate-950/50 p-3 text-left text-xs leading-5 text-slate-400"><b className="text-slate-200">QR CineBooking:</b> camera mặc định trên điện thoại có thể mở thẳng trang soát vé của CineBooking.{localOnly&&<div className="mt-2 text-amber-300">⚠ QR đang dùng {publicBase}. Điện thoại khác không truy cập được localhost. Hãy đặt TICKET_PUBLIC_BASE_URL thành IP LAN/miền nghiệp vụ của máy chủ rồi recreate backend.</div>}</div>}
