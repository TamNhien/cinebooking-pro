...
function Inner(){
  ...
  useEffect(()=>{if(!paymentId){setErr("Thiếu paymentId");return;}let active=true;const load=()=>api<PaymentCheckout>(`/payments/${paymentId}/checkout`).then(p=>{if(!active)return;setPayment(p);if(p.status==="SUCCESS")window.location.assign("/bookings")}).catch(e=>active&&setErr(e.message));load();const timer=setInterval(load,3000);return()=>{active=false;clearInterval(timer)};},[paymentId]);
  ...
  {payment&&<div className="mt-7 space-y-5">{payment.qrData?<div className="mx-auto w-fit rounded-2xl bg-white p-5"><QRCodeSVG value={payment.qrData} size={260} level="M"/></div>:<div className="rounded-xl bg-amber-950/40 p-4 text-amber-200">MoMo không trả về địa chỉ mã QR cho tài khoản này. Hãy dùng nút mở trang thanh toán.</div>}
  ...
  <p className="text-xs text-slate-500">Sau khi thanh toán, trạng thái đặt vé sẽ được cập nhật qua IPN/chuyển hướng từ MoMo.</p></div>}
