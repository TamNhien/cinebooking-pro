...
export default function ObservabilityV65Page(){
  ...
  return <div className="space-y-7" data-testid="observability-v65">
    ...
    <section className="card overflow-hidden" data-testid="recent-traces-v65">
      <div className="border-b border-slate-800 p-5"><h2 className="text-xl font-bold">Truy vết yêu cầu gần đây</h2><p className="mt-1 text-sm text-slate-500">Chỉ lưu ring-buffer in-memory của replica, path đã normalize ID/query để tránh high-cardinality và lộ dữ liệu. Dùng trace ID để grep log.</p></div>
    ...
    <div className="card p-5 text-sm text-slate-400"><b className="text-white">Trace nhanh:</b> lấy <code>{summary?.traceHeader??"X-Trace-Id"}</code> từ response rồi chạy <code>docker compose logs backend-1 backend-2 | Select-String &quot;&lt;TRACE_ID&gt;&quot;</code>. V65 không ghi token, query-string hay dữ liệu gửi đến vào request telemetry.</div>
  </div>;
}
