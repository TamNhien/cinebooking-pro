...
import { voucherDisplayName } from "@/lib/controlled-business-presentation";
...
export default function AdminVouchersPage(){
  ...
  return <div className="space-y-7">
    <div className="flex flex-wrap items-end justify-between gap-4">
      <div><div className="mb-2 text-sm text-slate-400"><Link href="/admin" className="hover:text-white">Quản trị viên</Link> / Mã ưu đãi</div><h1 className="text-3xl font-bold">Quản lý mã ưu đãi</h1><p className="mt-1 text-slate-400">Quản trị viên tạo, sửa, kích hoạt/tạm dừng và kiểm thử mã ưu đãi trước khi khách sử dụng. Mã phải là duy nhất; WELCOME10/CINE20K là dữ liệu mẫu có sẵn.</p></div>
      <div className="flex gap-2"><Link href="/admin/commerce" className="btn btn-secondary">🍿 Bắp nước</Link><Link href="/admin" className="btn btn-secondary">← Bảng điều khiển</Link></div>
    </div>
    ...
    <div className="grid gap-6 xl:grid-cols-[440px_1fr]">
      <form onSubmit={save} className="card h-fit space-y-4 p-5">
        ...
        <div><label className="mb-1.5 block text-sm text-slate-300">Tên chương trình</label><input className="input" value={voucherDisplayName(form.name,language)} onChange={e=>setForm({...form,name:e.target.value})} placeholder={t("Ưu đãi thành viên mới","New member promotion")} required/></div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-3">{filtered.map(v=>{const st=currentStatus(v);return <div key={v.id} className="card p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex flex-wrap items-center gap-2"><span className="rounded-lg bg-rose-500/15 px-2 py-1 text-base font-black text-rose-300">{v.code}</span><span className={`rounded-full px-2 py-1 text-xs ${st.cls}`}>{st.label}</span></div><div data-testid="voucher-name-r7" className="mt-2 text-lg font-bold" data-i18n-skip="true">{voucherDisplayName(v.name,language)}</div><div className="mt-2 text-sm text-slate-400">{v.discountType==="PERCENT"?`${t("Giảm","Discount")} ${v.discountValue}%`:`${t("Giảm","Discount")} ${currency(v.discountValue)}`} · {t("đơn từ","minimum order")} {currency(v.minOrderAmount)}{v.maxDiscount!=null?` · ${t("tối đa","maximum")} ${currency(v.maxDiscount)}`:""}</div><div className="mt-1 text-xs text-slate-500">{t("Đã dùng","Used")} {v.usedCount}{v.usageLimit!=null?`/${v.usageLimit}`:` / ${t("không giới hạn","unlimited")}`}{v.startsAt?` · ${t("từ","from")} ${dateTime(v.startsAt)}`:""}{v.endsAt?` · ${t("đến","until")} ${dateTime(v.endsAt)}`:""}</div></div><div className="flex flex-wrap gap-2"><button className="btn btn-secondary" onClick={()=>navigator.clipboard?.writeText(v.code)}>📋 Sao chép</button><button className="btn btn-secondary" onClick={()=>testVoucher(v)}>🧪 Kiểm tra</button><button className="btn btn-secondary" onClick={()=>edit(v)}>✏️ Sửa</button><button className="btn btn-secondary" onClick={()=>toggle(v)}>{v.active?"⏸ Tạm dừng":"▶ Kích hoạt"}</button></div></div></div>})}{filtered.length===0&&<div className="card p-8 text-center text-slate-400">Chưa có mã ưu đãi phù hợp.</div>}</div>
      </div>
    </div>
  </div>;
}
