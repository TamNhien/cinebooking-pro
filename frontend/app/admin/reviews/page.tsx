...
export default function AdminReviewsPage(){
  ...
  return <div className="space-y-6">
    ...
    <div className="space-y-3">{reviews.map(r=><article key={r.id} className="card p-5"><div className="flex flex-wrap justify-between gap-4"><div><b>{r.userName}</b><div><StarRating value={r.rating} readonly size="sm"/></div><div className="mt-1 text-xs text-slate-500">Mã phim: {r.movieId} · {dateTime(r.updatedAt)}</div></div><button onClick={()=>remove(r.id)} className="btn btn-secondary">Xoá</button></div>{r.comment&&<p className="mt-3 whitespace-pre-wrap text-slate-300">{r.comment}</p>}</article>)}</div>
