...
export default function OperationsControlCenterV59(){
  ...
  return <main className="space-y-6" data-testid="operations-control-center-v59">
    <section className="card p-5 sm:p-6">
      <div data-testid="operations-control-center-v58" className="sr-only">Trung tâm điều khiển vận hành · tương thích V58</div>
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <div className="text-xs font-bold uppercase tracking-[0.24em] text-cyan-300">Vận hành thời gian thực · V59</div>
          <h1 className="mt-2 text-3xl font-black">Trung tâm vận hành thời gian thực</h1>
          <p className="mt-2 max-w-4xl text-sm text-slate-400">V59 nâng V58 bằng STOMP WebSocket trên Redis Pub/Sub: thanh toán, đặt vé, thiết bị, nhân viên, hỗ trợ, kho và sự cố phát tín hiệu làm mới ngay; ảnh chụp dự phòng chỉ còn là lớp an toàn.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <label className="flex items-center gap-2 rounded-xl border border-slate-800 px-3 py-2 text-xs text-slate-300"><input data-testid="operations-control-auto-refresh-v58" type="checkbox" checked={autoRefresh} onChange={e=>setAutoRefresh(e.target.checked)}/> Làm mới dự phòng</label>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-3">
        ...
        {data&&<span data-testid="operations-control-live-v58" className="text-xs text-slate-500">● Ảnh chụp thời gian thực · dự phòng {data.pollAfterSeconds}s · {dateTime(data.generatedAt)}</span>}
        ...
        {lastRealtimeAt&&<span className="text-xs text-slate-500">sự kiện gần nhất {dateTime(lastRealtimeAt)}</span>}
      </div>
    ...
    {data&&<>
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5" data-testid="operations-control-summary-v58">
        ...
        <div className="card p-4"><div className="text-xs text-slate-400">Đặt vé / vé</div><div className="mt-1 text-2xl font-black">{number(data.todayConfirmedBookings)}</div><div className="text-xs text-slate-500">{number(data.todayTickets)} vé confirmed</div></div>
        ...
        <div className="card p-4"><div className="text-xs text-slate-400">Tín hiệu nghiêm trọng</div><div className="mt-1 text-2xl font-black">{number(criticalCount)}</div><div className="text-xs text-slate-500">mức nghiêm trọng hiệu lực theo thời gian thực</div></div>
      </section>

      <section className="card p-5 sm:p-6" data-testid="operations-control-domains-v58">
        <div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-xl font-bold">Nhịp vận hành</h2><p className="mt-1 text-sm text-slate-500">7 miền nghiệp vụ trên cùng một bề mặt điều khiển hướng sự kiện.</p></div><span className="text-sm text-slate-400">{data.cinemaName}</span></div>
        <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {data.domains.map(domain=><a key={domain.domain} href={domain.href} className={`rounded-2xl border p-4 transition hover:border-slate-500 ${statusClass(domain.status)}`}>
            ...
            <div className="mt-3 flex gap-4 text-sm"><div><div className="text-xs opacity-70">Chính</div><b>{number(domain.primaryCount)}</b></div><div><div className="text-xs opacity-70">Cảnh báo</div><b>{number(domain.warningCount)}</b></div></div>
          </a>)}
        </div>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.15fr_.85fr]">
        <div className="card p-5 sm:p-6">
          <div className="flex items-center justify-between gap-3"><div><h2 className="text-xl font-bold">Cảnh báo thời gian thực tập trung</h2><p className="mt-1 text-sm text-slate-500">Xác nhận giữ trạng thái 60 phút; đánh dấu đã xử lý sẽ tạm ẩn 15 phút. Cảnh báo ĐANG MỞ quá 10 phút tự nâng một bậc nghiêm trọng.</p></div><span className="text-xs text-slate-500">Trạng thái Redis + lịch sử kiểm toán</span></div>
          <div className="mt-5 space-y-3" data-testid="operations-control-alerts-v58">
            {data.alerts.length?data.alerts.map(item=><div key={item.fingerprint} className={`rounded-2xl border p-4 ${item.state==="RESOLVED"?"opacity-60 ":""}${severityClass(item.effectiveSeverity)}`}>
              <div className="flex items-start justify-between gap-4"><div><div className="flex flex-wrap gap-2 text-xs font-black uppercase tracking-wider"><span>{item.effectiveSeverity} · {item.domain}</span>{item.escalated&&<span className="rounded bg-rose-500/20 px-2 py-0.5">ĐÃ NÂNG MỨC</span>}<span className="rounded bg-slate-950/40 px-2 py-0.5">{alertStateLabel(item.state)}</span></div><a href={item.href} className="mt-1 block font-semibold text-slate-100 hover:underline">{item.title}</a><div className="mt-1 text-xs text-slate-400">{item.detail}</div><div className="mt-2 text-[11px] text-slate-500">Ghi nhận đầu tiên: {dateTime(item.firstSeenAt)}{item.stateActor?` · ${item.stateActor}`:""}</div></div><div className="rounded-xl bg-slate-950/50 px-3 py-1 text-lg font-black">{number(item.count)}</div></div>
            ...
            </div>):<div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5 text-sm text-emerald-200">Không có tín hiệu cần cảnh báo ở ảnh chụp hiện tại.</div>}
          </div>
        </div>

        <div className="space-y-6">
          <div className="card p-5 sm:p-6" data-testid="operations-control-detail-v58">
            <h2 className="text-xl font-bold">Chi tiết điều khiển</h2>
            ...
            <div className="mt-5 text-xs leading-5 text-slate-500">V59 dùng {data.realtimeTransport} topic <code>{data.realtimeTopic}</code>. Ảnh chụp dự phòng {data.pollAfterSeconds}s chỉ bảo vệ khi WebSocket mất kết nối; dữ liệu nghiệp vụ vẫn được tính trực tiếp từ cơ sở dữ liệu như V58.</div>
          </div>

          <div className="card p-5 sm:p-6" data-testid="operations-control-history-v59">
            <div className="flex items-center justify-between gap-3"><div><h2 className="text-xl font-bold">Lịch sử thao tác cảnh báo</h2><p className="mt-1 text-sm text-slate-500">Lịch sử ACK/Resolve được ghi vào audit_log, không tạo bảng giả mới.</p></div><button className="btn btn-secondary" type="button" onClick={()=>loadHistory()}>↻</button></div>
            <div className="mt-4 space-y-2">
              {history.length?history.map(item=><div key={item.id} className="rounded-xl border border-slate-800 p-3 text-xs"><div className="font-bold text-slate-200">{item.action==="OPS_ALERT_ACKNOWLEDGE"?"Tiếp nhận cảnh báo":"Đánh dấu đã xử lý"}</div><div className="mt-1 text-slate-400">{item.detail}</div><div className="mt-1 text-slate-600">{item.actorEmail||"system"} · {dateTime(item.createdAt)}</div></div>):<div className="text-sm text-slate-500">Chưa có thao tác cảnh báo trong phạm vi hiện tại.</div>}
            </div>
          </div>
        </div>
      </section>
