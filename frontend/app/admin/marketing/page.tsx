...
export default function MarketingAutomationV64Page(){
  ...
  async function launch(){
    if(!preview){setMsg("Hãy chạy Xem trước trước khi phát hành chiến dịch.");return;}
    if(preview.campaignCode!==form.campaignCode.trim().toUpperCase()||preview.segmentCode!==form.segmentCode){setMsg("Nội dung đã thay đổi sau Xem trước. Hãy Xem trước lại trước khi phát hành.");return;}
  ...
  return <div className="space-y-7" data-testid="marketing-v64">
    <div className="flex flex-wrap items-end justify-between gap-4">
      <div>
        <div className="mb-2 text-sm text-slate-400"><Link href="/admin" className="hover:text-white">Quản trị viên</Link> / CRM & tiếp thị</div>
        <div className="text-xs font-black tracking-[0.22em] text-rose-300">V64 · CRM & TỰ ĐỘNG HÓA TIẾP THỊ 4.0</div>
        <h1 className="mt-2 text-3xl font-bold">Segment → Chiến dịch → Voucher</h1>
        <p className="mt-1 max-w-4xl text-slate-400">Phân khúc trực tiếp từ tài khoản, lượt đặt vé và thanh toán thật; phát hành mã ưu đãi cá nhân dùng một lần và gửi khuyến mãi qua các kênh mà khách đã bật.</p>
      </div>
      <div className="flex gap-2"><Link href="/admin/vouchers" className="btn btn-secondary">🎟 Mã ưu đãi</Link><Link href="/admin" className="btn btn-secondary">← Bảng điều khiển</Link></div>
    </div>

    {msg&&<div className="card p-4 text-sm">{msg}</div>}

    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      <div className="card p-5"><div className="text-xs uppercase tracking-wider text-slate-500">Chiến lược</div><div className="mt-2 font-bold text-rose-300">{overview?.strategyVersion??"V64-CRM-AUTOMATION-4"}</div></div>
      ...
      <div className="card p-5"><div className="text-xs uppercase tracking-wider text-slate-500">Điều kiện gửi</div><div className="mt-2 font-bold text-emerald-300">Promotion từ chối nhận khuyến mãi respected</div></div>
    </div>

    <div className="grid gap-6 xl:grid-cols-[430px_1fr]">
      <form onSubmit={runPreview} className="card h-fit space-y-4 p-5" data-testid="campaign-composer-v64">
        <div><h2 className="text-xl font-bold">Tạo chiến dịch V64</h2><p className="mt-1 text-xs text-slate-500">Bắt buộc xem trước trước khi phát hành. Mã chiến dịch là khóa chống lặp: chạy lại cùng mã sẽ không phát mã ưu đãi/thông báo trùng.</p></div>
        ...
        <div><label className="mb-1.5 block text-sm text-slate-300">Hiệu lực mã ưu đãi (ngày)</label><input className="input" type="number" min={1} max={90} value={form.validityDays} onChange={e=>{setForm({...form,validityDays:Number(e.target.value)});setPreview(null)}}/></div>
        <div className="grid grid-cols-2 gap-2"><button className="btn btn-secondary" disabled={busy} data-testid="campaign-preview-v64">{busy?"Đang xử lý...":"🔎 Xem trước"}</button><button type="button" className="btn btn-primary" disabled={busy||!preview} onClick={launch} data-testid="campaign-launch-v64">🚀 Phát hành</button></div>
      </form>
      ...
      <div className="space-y-5">
        ...
        {preview&&<div className="card p-5" data-testid="campaign-preview-result-v64">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="text-xs font-black tracking-widest text-cyan-300">XEM TRƯỚC</div><h2 className="mt-1 text-xl font-bold">{preview.campaignCode} · {preview.segmentLabel}</h2></div><div className="rounded-xl bg-cyan-500/10 px-4 py-2 text-2xl font-black text-cyan-300">{preview.matchedCustomers}</div></div>
          <div className="mt-4 grid gap-2 text-xs text-slate-400"><div>🎟 {preview.voucherPolicy}</div><div>🔔 {preview.deliveryPolicy}</div></div>
          <div className="mt-5 overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead className="text-xs uppercase text-slate-500"><tr><th className="pb-2">Khách</th><th className="pb-2">Hạng</th><th className="pb-2">Đặt vé</th><th className="pb-2">Doanh thu</th><th className="pb-2">Độ gần đây</th></tr></thead><tbody className="divide-y divide-slate-800">{preview.audience.map(a=><tr key={a.customerRef}><td className="py-3"><div className="font-semibold">{a.customerRef}</div><div className="text-xs text-slate-500">{a.maskedEmail}</div></td><td className="py-3">{a.membershipTier}</td><td className="py-3">{a.lifetimeBookings}</td><td className="py-3">{currency(a.lifetimeRevenue)}</td><td className="py-3">{a.recencyDays<0?"Chưa đặt vé":`${a.recencyDays} ngày`}</td></tr>)}</tbody></table></div>
        ...
        {result&&<div className="card border border-emerald-700/60 p-5" data-testid="campaign-launch-result-v64"><div className="text-xs font-black tracking-widest text-emerald-300">ĐÃ PHÁT HÀNH · AN TOÀN KHI LẶP</div><h2 className="mt-1 text-xl font-bold">{result.campaignCode}</h2><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Metric label="Khớp" value={result.matchedCustomers}/><Metric label="Mã ưu đãi mới" value={result.vouchersCreated}/><Metric label="Mã ưu đãi tái sử dụng" value={result.vouchersReused}/><Metric label="Thông báo tạo" value={result.notificationsCreated}/></div><div className="mt-3 text-xs text-slate-500">Thông báo bỏ qua: {result.notificationsSkipped} (từ chối nhận khuyến mãi/kênh tắt hoặc trùng lặp). Mã ưu đãi cá nhân vẫn thuộc đúng tài khoản.</div></div>}
      </div>
    </div>
  </div>;
}
...
Hãy chạy Preview trước khi phát hành chiến dịch
*/
