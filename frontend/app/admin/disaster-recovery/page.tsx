...
async function withTransientDrReadRetry<T>(read:()=>Promise<T>){
...
export default function DisasterRecoveryPage(){
  ...
  const load=useCallback(async()=>{
    try{
      const [me,s,b,d]=await withTransientDrReadRetry(()=>Promise.all([
        api<UserProfile>("/me"),
  ...
  return <div className="mx-auto max-w-7xl space-y-6 px-4 py-8" data-testid="disaster-recovery-v69" data-dr-ready={summary?"true":"false"}>
    <div className="flex flex-wrap items-end justify-between gap-4">
      <div><p className="section-kicker">V69 · SAO LƯU & PHỤC HỒI SAU THẢM HỌA 5.0</p><h1 className="text-3xl font-black">Sao lưu & phục hồi sau thảm họa</h1><p className="mt-2 max-w-3xl text-slate-400">Bằng chứng lưu trữ PostgreSQL đã xác minh, lịch sử DR chỉ ghi thêm, diễn tập khôi phục không phá hủy và mức sẵn sàng RPO/RTO.</p></div>
    ...
    <section className="grid gap-4 lg:grid-cols-3">
      ...
      <div className="card p-5" data-testid="dr-drill-health-v69"><h2 className="text-lg font-black">🧪 Trạng thái diễn tập khôi phục</h2><div className="mt-4 space-y-2 text-sm text-slate-300"><div>Số lần diễn tập thành công: <b>{summary?.successfulDrillCount??0}</b></div><div>Thời gian từ diễn tập gần nhất: <b>{summary?.latestDrillAgeHours==null?"—":`${summary.latestDrillAgeHours} giờ`}</b></div><div>Diễn tập còn mới: <b>{summary?.drillFresh?"PASS":"NOT READY"}</b></div><div>Đạt RTO: <b>{summary?.rtoMet?"PASS":"NOT READY"}</b></div></div></div>
      <div className="card p-5" data-testid="dr-evidence-policy-v69" data-evidence-mode={summary?.immutableEvidence?"APPEND_ONLY":"LOADING"}><h2 className="text-lg font-black">🔒 Chính sách bằng chứng</h2><div className="mt-4 space-y-2 text-sm text-slate-300"><div>Bằng chứng CSDL: <b>{summary?.immutableEvidence?"APPEND-ONLY":"—"}</b></div><div>Payload lưu trữ: <b>không lưu trong DB</b></div><div>Tệp kê khai: <b>chỉ siêu dữ liệu</b></div><div>Danh mục trọng yếu: <b>{summary?.criticalCatalog?.length??0} bảng</b></div></div></div>
    </section>
