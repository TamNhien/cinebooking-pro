...
export default function PrivacyGovernancePage(){
  ...
  async function review(row:PrivacyRequestV70,decision:"APPROVED"|"REJECTED"|"CANCELLED"){
    ...
    try{
      ...
      setMessage(`${updated.requestKey} → ${localizedLabel(updated.status,language)}`);await load();
    }catch(e){setError((e as Error).message);}finally{setBusy("");}
  }
  ...
  return <div className="mx-auto max-w-7xl space-y-6 px-4 py-8" data-testid="privacy-governance-v70">
    ...
    <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5" data-testid="privacy-governance-summary-v70" data-retention-mode={summary?summary.dryRunOnly?"DRY_RUN":"EXECUTION_ENABLED":"LOADING"}>
      ...
      <div className="card p-5"><div className="text-xs font-bold uppercase text-slate-500">Chế độ lưu giữ</div><div className="mt-2 text-lg font-black">{summary?.dryRunOnly?t("CHỈ CHẠY THỬ","DRY RUN ONLY"):t("ĐÃ BẬT","ENABLED")}</div></div>
    </section>
    ...
    <section className="grid gap-4 lg:grid-cols-2">
      <div className="card p-5" data-testid="privacy-subject-inventory-v70">
        ...
        {inventory&&<div className="mt-4 space-y-2 text-sm"><div><b>{inventory.fullName}</b> · {inventory.email}</div><div>Tổng bản ghi liên quan: <b>{inventory.totalRelatedRecords}</b></div><div>Có thao tác phá hủy: <b>{inventory.destructiveActionPerformed?"CÓ":"KHÔNG"}</b></div><div className="mt-3 grid gap-2 sm:grid-cols-2">{inventory.items.map(item=><div className="rounded-xl border border-slate-800 p-3" key={item.source}><div className="font-bold">{item.source}</div><div className="text-slate-400">{localizedLabel(item.dataDomain,language)} · {localizedLabel(item.handling,language)}</div><div className="mt-1 text-lg font-black">{item.recordCount}</div></div>)}</div></div>}
      </div>
      ...
      <form className="card p-5" onSubmit={createRequest} data-testid="privacy-request-form-v70">
        ...
        <label className="mt-4 block text-xs font-bold uppercase text-slate-500">Loại yêu cầu</label><select className="input mt-2 w-full" value={requestType} onChange={e=>setRequestType(e.target.value as RequestType)}><option value="EXPORT">{t("Xuất dữ liệu","Data export")}</option><option value="ERASURE">{t("Xóa dữ liệu","Delete data")}</option><option value="RECTIFICATION">{t("Chỉnh sửa dữ liệu","Correct data")}</option></select>
    ...
    <section className="card overflow-x-auto p-5" data-testid="privacy-retention-policies-v70">
      ...
      <table className="mt-4 min-w-full text-sm"><thead className="text-xs uppercase text-slate-500"><tr><th className="p-2">Chính sách</th><th className="p-2">Phân loại</th><th className="p-2">Bảng</th><th className="p-2">Giữ chân khách hàng</th><th className="p-2">Thao tác</th><th className="p-2">Tự động phá hủy</th></tr></thead><tbody>{policies.map(p=><tr className="border-t border-slate-800" key={p.id}><td className="p-2 font-bold">{p.policyKey}</td><td className="p-2">{p.dataClass}</td><td className="p-2"><code>{p.tableName}</code></td><td className="p-2">{p.retentionDays} {t("ngày","days")}</td><td className="p-2">{localizedLabel(p.retentionAction,language)}</td><td className="p-2 font-bold">{p.destructiveExecutionEnabled?t("BẬT","ON"):t("TẮT","OFF")}</td></tr>)}</tbody></table>
    </section>
