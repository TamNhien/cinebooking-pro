...
export default function ShowtimePlannerPage(){
  ...
  const formatCurrency = (value:number) => currency(value, locale);
  ...
  return <div className="mx-auto max-w-7xl space-y-7" data-testid="showtime-planning-page">
    ...
    <section className="card p-5" data-testid="smart-showtime-planner">
      ...
      {smartPreview&&<div className="mt-5 space-y-4">
        ...
        <div className="grid gap-4 xl:grid-cols-2">{smartPreview.days.map(day=><article key={day.date} className="rounded-xl border border-slate-800 bg-slate-950/40 p-4"><div className="flex items-center justify-between"><div><b>{day.date}</b><div className="text-xs text-slate-500">{t(`${day.suggested}/${day.target} suất · ${day.conflicts} ứng viên xung đột`,`${day.suggested}/${day.target} showtimes · ${day.conflicts} conflicting candidates`)}</div></div></div><div className="mt-3 space-y-2">{day.slots.map(slot=><div key={`${slot.auditoriumId}-${slot.startTime}`} className="rounded-lg border border-emerald-900/50 bg-emerald-950/20 p-3 text-sm"><div className="flex flex-wrap justify-between gap-2"><b>{formatDateTime(slot.startTime)} · {auditoriumDisplayName(slot.auditoriumName, language)}</b><span className="font-bold text-emerald-300">{t(`Điểm ${slot.score}`,`Score ${slot.score}`)}</span></div><div className="mt-1 text-xs text-slate-400">→ {formatDateTime(slot.endTime)}{slot.historicalSamples>0?t(` · lịch sử ${slot.historicalOccupancy}% / ${slot.historicalSamples} suất`,` · historical ${slot.historicalOccupancy}% / ${slot.historicalSamples} showtimes`):t(" · nhu cầu heuristic"," · heuristic demand")}</div><div className="mt-1 text-xs text-slate-500">{slot.reasons.map(reason=>plannerReason(reason,language)).join(" · ")}</div></div>)}{!day.slots.length&&<div className="text-sm text-amber-300">{t("Không đủ khoảng trống phù hợp trong ngày này.","No suitable time slots are available on this day.")}</div>}</div></article>)}</div>
      </div>}
    </section>

    <section className="card p-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-xl font-bold">{t("Lịch sử bộ lập lịch thông minh","Smart Planner run history")}</h2><p className="text-sm text-slate-500">{t("20 lần xác nhận gần nhất · dùng để kiểm toán nguồn gốc của suất THÔNG MINH.","20 most recent confirmed runs · used to audit SMART showtime provenance.")}</p></div></div><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[850px] text-sm"><thead className="text-xs uppercase text-slate-500"><tr><th className="px-2 pb-3">{t("Lần chạy","Run ID")}</th><th className="px-2 pb-3">{t("Rạp / phim","Cinema / movie")}</th><th className="px-2 pb-3">{t("Khoảng ngày","Date range")}</th><th className="px-2 pb-3">{t("Mục tiêu","Target")}</th><th className="px-2 pb-3">{t("Tạo","Created")}</th><th className="px-2 pb-3">{t("Xung đột","Conflicts")}</th><th className="px-2 pb-3">{t("Người tạo","Created by")}</th></tr></thead><tbody className="divide-y divide-slate-800">{runs.map(r=><tr key={r.id} data-testid="smart-planning-run"><td className="p-2 font-mono text-xs">{r.id.slice(0,8)}</td><td className="p-2"><b>{r.cinemaName}</b><div className="text-xs text-slate-500">{r.movieTitle}</div></td><td className="p-2">{r.fromDate} → {r.toDate}</td><td className="p-2">{r.requestedSlots}</td><td className="p-2 text-emerald-300">{r.suggestedSlots}</td><td className="p-2">{r.conflictCount}</td><td className="p-2">{r.createdBy||"-"}</td></tr>)}{!runs.length&&<tr><td colSpan={7} className="py-8 text-center text-slate-500">{t("Chưa có lần lập kế hoạch V49.","No V49 planning runs yet.")}</td></tr>}</tbody></table></div></section>
    ...
    <div className="admin-split-grid">
      <form onSubmit={runPreview} className="card space-y-4 p-5">
        ...
        {selectedMovie&&selectedRoom&&<div className="rounded-xl bg-slate-950/60 p-3 text-xs text-slate-400">{t("Đang lập", "Planning")}: <b className="text-slate-200">{selectedMovie.title}</b> {t("tại","at")} <b className="text-slate-200">{selectedRoom.cinemaName} · {auditoriumDisplayName(selectedRoom.name, language)}</b>.</div>}
      </form>
