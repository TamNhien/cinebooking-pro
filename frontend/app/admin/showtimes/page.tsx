...
export default function ShowtimePlannerPage(){
  ...
  async function commitSmart(){
    ...
    try{
      ...
      setMsg(`V49 Bộ lập lịch thông minh đã tạo ${result.created} suất · run ${result.planningRunId.slice(0,8)}.`);
  ...
  return <div className="mx-auto max-w-7xl space-y-7">
    <div className="flex flex-wrap items-end justify-between gap-4">
      <div><p className="section-kicker">V49 · LẬP LỊCH SUẤT CHIẾU THÔNG MINH 2.0</p><h1 className="text-3xl font-bold">Lập lịch chiếu & chống trùng phòng</h1><p className="mt-2 max-w-4xl text-slate-400">Smart Planner xếp suất theo nhu cầu lịch sử, giờ cao điểm và cuối tuần; đồng thời loại trừ lịch phòng, thời gian dọn phòng và blackout/bảo trì trước khi đề xuất.</p></div>
      <div className="flex gap-2"><Link href="/admin/maintenance" className="btn btn-secondary">🛠 Bảo trì phòng</Link><Link href="/admin" className="btn btn-secondary">← Bảng điều khiển quản trị</Link></div>
    </div>
    ...
    <section className="card p-5" data-testid="smart-showtime-planner">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-2xl font-bold">✨ Lập lịch thông minh</h2><p className="mt-1 text-sm text-slate-500">Cân bằng theo nhu cầu · không ghi cơ sở dữ liệu khi xem trước · xác nhận sẽ lưu nguồn gốc và lần lập kế hoạch.</p></div><span className="rounded-full border border-indigo-800 bg-indigo-950/40 px-3 py-1 text-xs text-indigo-200">V49-DEMAND-BALANCED-2</span></div>
      <form onSubmit={runSmartPreview} className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
        ...
        <label className="text-sm"><span className="mb-1 block text-slate-400">Phim</span><select data-testid="smart-movie-select" aria-label="Phim cho bộ lập lịch thông minh" className="input" value={smartForm.movieId} onChange={e=>{setSmartForm({...smartForm,movieId:e.target.value});setSmartPreview(null)}} required><option value="">Chọn phim</option>{movies.filter(m=>m.active).map(m=><option key={m.id} value={m.id}>{m.title} · {m.durationMinutes} phút</option>)}</select></label>
      ...
      {smartPreview&&<div className="mt-5 space-y-4">
        <div className="grid gap-3 sm:grid-cols-3 xl:grid-cols-6">
          <Metric label="Mục tiêu" value={smartPreview.requested}/><Metric testId="smart-suggested-metric" label="Đề xuất" value={smartPreview.suggested}/><Metric label="Ứng viên" value={smartPreview.candidateCount}/><Metric label="Bị chặn" value={smartPreview.conflicts}/><Metric label="Mẫu lịch sử" value={smartPreview.historicalSamples}/><Metric label="Dọn phòng" value={`${smartPreview.turnaroundMinutes} phút`}/>
        </div>
        <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-xs text-slate-500">Múi giờ {smartPreview.zoneId} · điểm 0–100 kết hợp tỷ lệ lấp đầy lịch sử và heuristic nhu cầu.</p><button data-testid="smart-commit-button" className="btn btn-primary" onClick={commitSmart} disabled={smartBusy||smartPreview.suggested===0}>Tạo {smartPreview.suggested} suất gợi ý</button></div>
        <div className="grid gap-4 xl:grid-cols-2">{smartPreview.days.map(day=><article key={day.date} className="rounded-xl border border-slate-800 bg-slate-950/40 p-4"><div className="flex items-center justify-between"><div><b>{day.date}</b><div className="text-xs text-slate-500">{day.suggested}/{day.target} suất · {day.conflicts} candidate xung đột</div></div></div><div className="mt-3 space-y-2">{day.slots.map(slot=><div key={`${slot.auditoriumId}-${slot.startTime}`} className="rounded-lg border border-emerald-900/50 bg-emerald-950/20 p-3 text-sm"><div className="flex flex-wrap justify-between gap-2"><b>{dateTime(slot.startTime)} · {slot.auditoriumName}</b><span className="font-bold text-emerald-300">Điểm {slot.score}</span></div><div className="mt-1 text-xs text-slate-400">→ {dateTime(slot.endTime)}{slot.historicalSamples>0?` · lịch sử ${slot.historicalOccupancy}% / ${slot.historicalSamples} suất`:" · heuristic demand"}</div><div className="mt-1 text-xs text-slate-500">{slot.reasons.join(" · ")}</div></div>)}{!day.slots.length&&<div className="text-sm text-amber-300">Không đủ khoảng trống phù hợp trong ngày này.</div>}</div></article>)}</div>
      </div>}
    </section>

    <section className="card p-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-xl font-bold">Lịch sử Bộ lập lịch thông minh</h2><p className="text-sm text-slate-500">20 lần xác nhận gần nhất · dùng để kiểm toán nguồn gốc của suất SMART.</p></div></div><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[850px] text-sm"><thead className="text-left text-xs uppercase text-slate-500"><tr><th className="pb-3">Lần chạy</th><th>Rạp / phim</th><th>Khoảng ngày</th><th>Mục tiêu</th><th>Tạo</th><th>Xung đột</th><th>Người tạo</th></tr></thead><tbody className="divide-y divide-slate-800">{runs.map(r=><tr key={r.id} data-testid="smart-planning-run"><td className="py-3 font-mono text-xs">{r.id.slice(0,8)}</td><td><b>{r.cinemaName}</b><div className="text-xs text-slate-500">{r.movieTitle}</div></td><td>{r.fromDate} → {r.toDate}</td><td>{r.requestedSlots}</td><td className="text-emerald-300">{r.suggestedSlots}</td><td>{r.conflictCount}</td><td>{r.createdBy||"-"}</td></tr>)}{!runs.length&&<tr><td colSpan={7} className="py-8 text-center text-slate-500">Chưa có lần lập kế hoạch V49.</td></tr>}</tbody></table></div></section>

    <div className="grid gap-6 xl:grid-cols-[430px_1fr]">
      <form onSubmit={runPreview} className="card space-y-4 p-5">
        <div><p className="section-kicker">LÔ THỦ CÔNG · TƯƠNG THÍCH V34</p><h2 className="text-xl font-bold">Kế hoạch hàng loạt thủ công</h2><p className="mt-1 text-sm text-slate-500">Giữ nguyên quy trình xem trước/xác nhận cũ; suất tạo từ đây được đánh dấu Theo lô.</p></div>
      ...
      <div className="space-y-5">
        <section className="card p-5">
          <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-bold">Kết quả xem trước thủ công</h2><p className="text-sm text-slate-500">Xanh = có thể tạo · Đỏ = xung đột.</p></div>{preview&&<button className="btn btn-primary" onClick={commit} disabled={busy||preview.creatable===0||(!form.skipConflicts&&preview.conflicts>0)}>{!form.skipConflicts&&preview.conflicts>0?"Có xung đột cần xử lý":`Tạo ${preview.creatable} suất hợp lệ`}</button>}</div>
          {!preview?<div className="py-10 text-center text-slate-500">Chọn phim, phòng, ngày và bấm Xem trước lịch.</div>:<><div className="mt-4 grid gap-3 sm:grid-cols-4"><Metric label="Yêu cầu" value={preview.requested}/><Metric label="Có thể tạo" value={preview.creatable}/><Metric label="Trùng lịch" value={preview.conflicts}/><Metric label="Dọn phòng" value={`${preview.turnaroundMinutes} phút`}/></div><p className="mt-3 text-xs text-slate-500">Múi giờ: {preview.zoneId}</p><div className="mt-4 max-h-[430px] space-y-2 overflow-auto pr-1">{preview.slots.map(slot=><div key={slot.startTime} className={`rounded-xl border p-3 text-sm ${slot.creatable?"border-emerald-900/60 bg-emerald-950/20":"border-red-900/60 bg-red-950/20"}`}><div className="flex flex-wrap items-center justify-between gap-2"><b>{dateTime(slot.startTime)} → {dateTime(slot.endTime)}</b><span className={slot.creatable?"text-emerald-300":"text-red-300"}>{slot.creatable?"Có thể tạo":"Trùng lịch"}</span></div>{slot.conflictLabel&&<div className="mt-1 text-xs text-red-200">Xung đột: {slot.conflictLabel}</div>}</div>)}</div></>}
        </section>
        <section className="card p-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-xl font-bold">Lịch hiện có trong phòng</h2><p className="text-sm text-slate-500">{roomSchedule.length} suất trong bộ lọc hiện tại.</p></div><span className="text-xs text-slate-500">Nguồn gốc THỦ CÔNG / LÔ / THÔNG MINH</span></div><div className="mt-4 max-h-[420px] space-y-2 overflow-auto">{roomSchedule.map(s=><div key={s.id} className="rounded-xl border border-slate-800 bg-slate-950/50 p-3 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><b>{s.movieTitle}</b><span className="text-slate-400">{s.status} · {s.planningSource||"MANUAL"}{s.planningScore!=null?` · ${s.planningScore}`:""}</span></div><div className="mt-1 text-slate-400">{dateTime(s.startTime)} · {s.cinemaName}/{s.auditoriumName} · {currency(s.basePrice)}</div></div>)}{!roomSchedule.length&&<div className="py-8 text-center text-slate-500">Chưa có suất phù hợp bộ lọc.</div>}</div></section>
      </div>
    </div>
  </div>;
}
