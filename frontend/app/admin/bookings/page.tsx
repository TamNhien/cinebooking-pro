    ...
    <div className="admin-booking-modal-overlay" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)closeDetail()}}>
      <section className="admin-booking-modal" role="dialog" aria-modal="true" aria-labelledby="admin-booking-modal-title">
        <div className="admin-booking-modal-header">
          <div><div className="text-xs text-rose-400">MÃ ĐẶT VÉ #{selected.id}</div><h2 id="admin-booking-modal-title" data-testid="admin-booking-modal-movie-title-v7817" data-i18n-skip="true" className="text-2xl font-black">{selected.movieTitle}</h2></div>
        ...
        <div className="admin-booking-modal-scroll">
          <div className="admin-booking-modal-content grid gap-5 lg:grid-cols-[1.35fr_.8fr]">
            ...
            <div className="space-y-5">
              ...
              <section className="card p-5"><h3 className="mb-3 font-black">Thanh toán</h3>{selected.payments.length?selected.payments.map(p=><div key={p.id} className="mb-3 rounded-xl border border-slate-800 p-3 text-sm"><div className="flex justify-between"><b>{p.provider}</b><span className={`rounded-full border px-2 py-1 text-xs ${badge(p.status)}`}>{viLabel(p.status)}</span></div><div className="mt-2 text-slate-400">{currency(p.amount)} · {dateTime(p.createdAt)}</div>{p.providerTransactionId&&<div className="break-all text-xs text-slate-500">Mã giao dịch: {p.providerTransactionId}</div>}</div>):<div className="text-slate-500">Đặt vé chưa có thanh toán.</div>}</section>
