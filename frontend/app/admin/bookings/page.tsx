...
export default function AdminBookingsPage(){
  ...
  async function refundRequest(){if(!selected)return;const reason=prompt("Lý do hoàn vé (không bắt buộc):","Quản trị viên hỗ trợ khách hàng hoàn vé");if(reason===null)return;await action(`/admin/booking-ops/${selected.id}/refund-request`,{reason},"Tạo yêu cầu hoàn tiền cho booking này?")}
  async function approveRefund(){
    ...
    if(provider!=="MOCK"){
      ...
      if(!entered.trim()){setMsgKind("error");setMsg("Cần nhập mã tham chiếu hoàn tiền từ cổng thanh toán trước khi duyệt.");return;}
  ...
  async function cancelPending(){if(!selected)return;const reason=prompt("Lý do hủy lượt đặt vé đang chờ:","Quản trị viên hủy đơn chưa thanh toán");if(reason===null)return;await action(`/admin/booking-ops/${selected.id}/cancel`,{reason},"Huỷ booking và mở lại ghế?")}
    ...
    <div className="admin-booking-modal-overlay" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)closeDetail()}}>
      <section className="admin-booking-modal" role="dialog" aria-modal="true" aria-labelledby="admin-booking-modal-title">
        <div className="admin-booking-modal-header">
          <div><div className="text-xs text-rose-400">{t("MÃ ĐẶT VÉ","BOOKING CODE")} #{selected.id}</div><h2 id="admin-booking-modal-title" data-testid="admin-booking-modal-movie-title-v7817" data-i18n-skip="true" className="text-2xl font-black">{selected.movieTitle}</h2></div>
        ...
        <div className="admin-booking-modal-scroll">
          <div className="admin-booking-modal-content grid gap-5 lg:grid-cols-[1.35fr_.8fr]">
            ...
            <div className="space-y-5">
              <section className="card p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-black">Khách hàng</h3><div>{selected.customerName}</div><div className="text-sm text-slate-400">{selected.customerEmail} · {selected.customerPhone||"-"}</div></div><span className={`rounded-full border px-3 py-1 text-xs font-black ${badge(selected.status)}`}>{viLabel(selected.status)}</span></div></section>
              <section className="card p-5"><h3 className="mb-3 font-black">Vé & suất chiếu</h3><div className="grid gap-2 text-sm sm:grid-cols-2"><div><span className="text-slate-400">Rạp:</span> {selected.cinemaName}</div><div><span className="text-slate-400">Phòng:</span> {auditoriumDisplayName(selected.auditoriumName, language)}</div><div><span className="text-slate-400">Suất:</span> {dateTime(selected.showtimeStart)}</div><div><span className="text-slate-400">Ghế:</span> {selected.seats.map(s=>s.code).join(", ")||"-"}</div><div><span className="text-slate-400">Tạo:</span> {dateTime(selected.createdAt)}</div><div><span className="text-slate-400">Soát vé:</span> {selected.checkedInAt?dateTime(selected.checkedInAt):"Chưa"}</div></div></section>
              <section className="card p-5"><h3 className="mb-3 font-black">Chi tiết tiền</h3><div className="space-y-2 text-sm"><div className="flex justify-between"><span>Tiền vé</span><b>{currency(selected.seatAmount)}</b></div><div className="flex justify-between"><span>Bắp nước</span><b>{currency(selected.concessionAmount)}</b></div>{selected.concessions.map((c,i)=><div className="flex justify-between pl-4 text-slate-400" key={i}><span>{concessionProductName(c.name,language)} × {c.quantity}</span><span>{currency(c.subtotal)}</span></div>)}<div className="flex justify-between text-emerald-300"><span>Giảm giá {selected.voucherCode?`(${selected.voucherCode})`:""}</span><b>-{currency(selected.discountAmount)}</b></div><div className="flex justify-between"><span>Điểm đã dùng</span><b>{selected.pointsRedeemed}</b></div><div className="mt-2 flex justify-between border-t border-slate-700 pt-3 text-lg"><b>Tổng</b><b>{currency(selected.totalAmount)}</b></div></div></section>
              <section className="card p-5"><h3 className="mb-3 font-black">Thanh toán</h3>{selected.payments.length?selected.payments.map(p=><div key={p.id} className="mb-3 rounded-xl border border-slate-800 p-3 text-sm"><div className="flex justify-between"><b>{p.provider}</b><span className={`rounded-full border px-2 py-1 text-xs ${badge(p.status)}`}>{viLabel(p.status)}</span></div><div className="mt-2 text-slate-400">{currency(p.amount)} · {dateTime(p.createdAt)}</div>{p.providerTransactionId&&<div className="break-all text-xs text-slate-500">{t("Mã giao dịch","Transaction ID")}: {p.providerTransactionId}</div>}</div>):<div className="text-slate-500">Đặt vé chưa có thanh toán.</div>}</section>
              <section className="card p-5"><h3 className="mb-3 font-black">Lịch sử quản trị / Kiểm toán</h3><div className="space-y-3">{selected.timeline.map(t=><div key={t.id} className="border-l-2 border-slate-700 pl-3 text-sm"><div className="font-bold">{t.action}</div><div className="text-xs text-slate-400">{dateTime(t.createdAt)} · {t.actorEmail||"system"}{t.ipAddress?` · ${t.ipAddress}`:""}</div>{t.details&&<div className="mt-1 text-slate-300">{t.details}</div>}</div>)}{selected.timeline.length===0&&<div className="text-sm text-slate-500">Chưa có kiểm toán trực tiếp cho lượt đặt vé này.</div>}</div></section>
            </div>
            <aside className="admin-booking-modal-actions space-y-4">
              <div className="card p-5"><h3 className="mb-4 font-black">Thao tác quản trị</h3><div className="grid gap-2">
                {selected.status==="CONFIRMED"&&<button className="btn btn-primary w-full" disabled={!!busy} onClick={showQr}>🎫 Xem QR vé</button>}
                {selected.status==="CONFIRMED"&&<button className="btn btn-secondary w-full" disabled={!!busy} onClick={()=>action(`/admin/booking-ops/${selected.id}/resend-ticket`,undefined,"Gửi lại vé qua thư điện tử khách hàng?")}>📧 Gửi lại vé qua thư điện tử</button>}
                {selected.status==="CONFIRMED"&&!selected.checkedInAt&&<button data-testid="admin-manual-checkin" className="btn btn-secondary w-full" disabled={!!busy} onClick={()=>action(`/admin/booking-ops/${selected.id}/manual-checkin`,undefined,"Check-in thủ công booking này? Admin có thể override khung giờ sau khi đã xác minh khách tại rạp.")}>✅ Soát vé thủ công</button>}
                {selected.checkedInAt&&<div data-testid="admin-manual-checkin-result" className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-3 text-sm text-emerald-200"><div className="font-black">✅ Đã soát vé</div><div className="mt-1">{dateTime(selected.checkedInAt)}</div>{selected.checkedInByEmail&&<div className="mt-1 text-xs text-emerald-300/80">{t("Bởi","By")} {selected.checkedInByEmail}</div>}</div>}
              ...
              </div><p className="text-xs leading-5 text-slate-500">Không xoá cứng lượt đặt vé. Hủy/hoàn tiền giữ nguyên thanh toán, soát vé và kiểm toán để đối soát.</p></div>
              {selected.refundReason&&<div className="card p-5"><h3 className="font-black">Hoàn tiền</h3><div className="mt-2 text-sm">{selected.refundReason}</div>{selected.refundAmount!==undefined&&<div className="mt-2 font-bold text-amber-300">{currency(selected.refundAmount)}</div>}</div>}
              {ticket&&<div className="card p-5 text-center"><h3 className="mb-3 font-black">QR vé</h3><img src={ticket.qrImageDataUrl} alt="QR vé" className="mx-auto w-full max-w-[320px] rounded-2xl bg-white p-3"/><div className="mt-3 break-all text-left text-xs text-slate-400">{ticket.qrUrl}</div><button className="btn btn-secondary mt-3 w-full" onClick={()=>navigator.clipboard?.writeText(ticket.qrUrl)}>Sao chép liên kết QR</button></div>}
            </aside>
          </div>
        </div>
      </section>
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><div className="text-xs font-black uppercase tracking-[.25em] text-rose-400">Vận hành đặt vé · V13</div><h1 className="text-3xl font-black">Quản lý đặt vé</h1><p className="text-slate-400">Đặt vé, thanh toán, QR, soát vé và hoàn tiền trên cùng một màn hình.</p></div><div className="flex gap-2"><Link href="/admin" className="btn btn-secondary">← Bảng điều khiển</Link><Link href="/admin/refunds" className="btn btn-secondary">↩ Hàng đợi hoàn vé</Link></div></div>
    ...
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="card p-4"><div className="text-xs text-slate-400">Tổng lượt đặt vé</div><div className="text-2xl font-black">{stats.total}</div></div><div className="card p-4"><div className="text-xs text-slate-400">ĐÃ XÁC NHẬN</div><div className="text-2xl font-black text-emerald-300">{stats.confirmed}</div></div><div className="card p-4"><div className="text-xs text-slate-400">Chờ hoàn tiền</div><div className="text-2xl font-black text-amber-300">{stats.refund}</div></div><div className="card p-4"><div className="text-xs text-slate-400">Giá trị thanh toán THÀNH CÔNG</div><div className="text-2xl font-black">{currency(stats.revenue)}</div></div></div>
    <div className="card grid gap-3 p-4 md:grid-cols-4"><input className="input" placeholder="Mã đặt vé, khách, phim, ghế..." value={q} onChange={e=>setQ(e.target.value)}/><select className="input" value={status} onChange={e=>setStatus(e.target.value)}>{statuses.map(x=><option key={x}>{x}</option>)}</select><select className="input" value={payment} onChange={e=>setPayment(e.target.value)}>{paymentStatuses.map(x=><option key={x}>{x}</option>)}</select><select className="input" value={cinema} onChange={e=>setCinema(e.target.value)}><option>TẤT CẢ</option>{cinemas.map(x=><option key={x} data-testid="admin-bookings-cinema-option-v7817" data-i18n-skip="true">{x}</option>)}</select></div>
    <div className="card overflow-x-auto p-3">{loading?<div className="p-6 text-slate-400">Đang tải lượt đặt vé...</div>:<table className="w-full min-w-[1050px] text-sm"><thead><tr className="text-slate-400"><th className="p-3">Đặt vé / Khách</th><th className="p-3">Phim / Rạp</th><th className="p-3">Suất & ghế</th><th className="p-3">Đặt vé</th><th className="p-3">Thanh toán</th><th className="p-3">Tổng</th><th className="p-3">Thao tác</th></tr></thead><tbody>{filtered.map(x=><tr key={x.id} className="border-t border-slate-800 align-top"><td className="p-3"><button className="font-bold text-rose-300 hover:underline" onClick={()=>openDetail(x.id)}>#{x.id.slice(0,8)}</button><div>{x.customerName}</div><div className="text-xs text-slate-400">{x.customerEmail}</div></td><td className="p-3"><b>{x.movieTitle}</b><div className="text-xs text-slate-400">{x.cinemaName} · {auditoriumDisplayName(x.auditoriumName, language)}</div></td><td className="p-3">{dateTime(x.showtimeStart)}<div className="text-xs text-slate-400">Ghế {x.seats.map(s=>s.code).join(", ")||"-"}</div></td><td className="p-3"><span className={`rounded-full border px-2 py-1 text-xs font-bold ${badge(x.status)}`}>{viLabel(x.status)}</span>{x.checkedInAt&&<div className="mt-2 text-xs text-emerald-300">✓ Đã soát vé</div>}</td><td className="p-3">{x.latestPayment?<><span className={`rounded-full border px-2 py-1 text-xs font-bold ${badge(x.latestPayment.status)}`}>{viLabel(x.latestPayment.status)}</span><div className="mt-2 text-xs text-slate-400">{x.latestPayment.provider}</div></>:<span className="text-slate-500">Chưa có</span>}</td><td className="p-3 font-bold">{currency(x.totalAmount)}</td><td className="p-3"><button className="btn btn-primary !px-3 !py-2" onClick={()=>openDetail(x.id)}>Quản lý</button></td></tr>)}{filtered.length===0&&<tr><td colSpan={7} className="p-8 text-center text-slate-500">Không có lượt đặt vé phù hợp.</td></tr>}</tbody></table>}</div>
