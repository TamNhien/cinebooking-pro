...
export default function AdminFinancePage(){
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div className="card p-4"><div className="text-xs text-slate-400">Lần chốt gần nhất</div><div data-testid="finance-run-status" data-run-status={data.latestRun?.status||"NOT_RUN"} className="text-2xl font-black">{data.latestRun?viLabel(data.latestRun.status):"CHƯA CHẠY"}</div><div className="text-xs text-slate-500">{data.latestRun?`${data.latestRun.issueCount} vấn đề · ${data.latestRun.loyaltyUsersChecked} tài khoản thành viên`:"-"}</div></div>
    </div>
