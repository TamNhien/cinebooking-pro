...
export default function ActionsRuntimeV73Page(){
  ...
  return <div className="space-y-7" data-testid="actions-runtime-v73">
    <div className="flex flex-wrap items-end justify-between gap-4">
      <div>
        <div className="mb-2 text-sm text-slate-400"><Link href="/admin" className="hover:text-white">Admin</Link> / GitHub Actions Runtime</div>
        <div className="text-xs font-black tracking-[0.22em] text-cyan-300">V73 · GITHUB ACTIONS RUNTIME MODERNIZATION 5.0</div>
        <h1 className="mt-2 text-3xl font-bold">Node 24 CI/CD Baseline</h1>
        <p className="mt-1 max-w-4xl text-slate-400">Bề mặt quản trị chỉ hiển thị baseline tooling đã khóa trong source. Không đọc GitHub token, không hiển thị secret và không gọi GitHub API từ trình duyệt.</p>
      </div>
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4" data-testid="actions-runtime-summary-v73">
      <Metric label="Strategy" value="V73-GITHUB-ACTIONS-NODE24-5"/>
      <Metric label="Runtime posture" value="NODE24 READY" tone="text-emerald-300"/>
      <Metric label="Flyway" value="V72"/>
      <Metric label="Public tables" value="67"/>
    </section>

    <section className="card overflow-hidden" data-testid="actions-runtime-baseline-v73">
      <div className="border-b border-slate-800 p-5">
        <h2 className="text-xl font-bold">GitHub Actions baseline</h2>
        <p className="mt-1 text-sm text-slate-500">Các major dưới đây là baseline V73. Regression verifier fail nếu action Node-20-era quay trở lại.</p>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full min-w-[680px] text-left text-sm">
          <thead className="bg-slate-900/70 text-xs uppercase text-slate-500"><tr><th className="p-3">Purpose</th><th className="p-3">Action</th><th className="p-3">Runtime</th></tr></thead>
    ...
    <section className="grid gap-5 lg:grid-cols-2">
      <div className="card p-5" data-testid="actions-runtime-policy-v73">
        ...
        <div className="mt-4 space-y-2 text-sm text-slate-300">
          ...
          <p>✅ Không dùng <code>FORCE_JAVASCRIPT_ACTIONS_TO_NODE24</code> để che action cũ.</p>
          <p>✅ Self-hosted runner baseline: <code>2.327.1+</code>.</p>
        </div>
      </div>
      <div className="card p-5" data-testid="actions-runtime-verify-v73">
        <h2 className="text-xl font-bold">Verification</h2>
        <p className="mt-2 text-sm text-slate-400">Nguồn sự thật vẫn là source verifier và GitHub CI của đúng commit, không phải trạng thái giả lập trên UI.</p>
        ...
        <div className="mt-3 text-xs text-slate-500">V73 là tooling-only: không migration, không backend mutation, không secret browser storage.</div>
      </div>
    </section>
  </div>;
}
