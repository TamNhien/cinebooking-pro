...
export default function PaymentResilienceV67(){
  ...
  return <div className="space-y-6" data-testid="payment-resilience-v67">
    ...
    <section className="card p-5" data-testid="payment-resilience-strategy-v67"><div className="grid gap-3 md:grid-cols-4"><Info label={t("Chiến lược","Strategy")} value={summary?.strategyVersion||"V67-PAYMENT-RESILIENCE-5"}/><Info label={t("Tự động đối soát","Automatic reconciliation")} value={localizedLabel(summary===null?"UNKNOWN":summary.autoReconcileEnabled?"ENABLED":"DISABLED",language)}/><Info label={t("Khôi phục thông báo máy chủ","Webhook recovery")} value={localizedLabel(summary===null?"UNKNOWN":summary.webhookRecoveryEnabled?"ENABLED":"DISABLED",language)}/><Info label={t("Máy chủ","Server")} value={summary?.evaluatedAt?dateTime(summary.evaluatedAt):"—"}/></div><div className="mt-4 rounded-xl border border-slate-800 bg-slate-950/50 p-4 text-sm leading-6 text-slate-300"><strong>{t("Chính sách khôi phục:","Recovery policy:")}</strong> {t("thông báo máy chủ hợp lệ nhưng trạng thái mồ côi/đang chờ chỉ được phục hồi bằng cách liên kết thanh toán theo mã đơn phía nhà cung cấp rồi truy vấn VNPay/MoMo. Dữ liệu gửi đến cũ không được phát lại như nguồn sự thật. Khoảng lùi có giới hạn; quá số lần thử sẽ chuyển","valid webhooks in orphaned/pending state are recovered only by linking payments through the provider order code and querying VNPay/MoMo. Old payloads are not replayed as source of truth. Backoff is bounded; exceeding the attempt limit moves the item to")} <code>DEAD_LETTER</code> {t("để quản trị viên xử lý.","for administrator handling.")}</div></section>
