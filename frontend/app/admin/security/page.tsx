...
export default function AdminSecurity(){
 ...
 return <div className="mx-auto max-w-7xl space-y-6 px-4 py-8" data-testid="security-identity-v68">
  <div><p className="section-kicker">V68 · BẢO MẬT & ĐỊNH DANH 5.0</p><h1 className="text-3xl font-black">Vận hành bảo mật · Bảo mật & định danh</h1><p className="mt-2 text-slate-400">Xác thực tăng cường cho thao tác quản trị nhạy cảm, quyền ngắn hạn gắn với phiên và chuẩn nền header bảo mật cho HTTPS.</p></div>
  ...
  <section className="card p-5" data-testid="admin-step-up-v68">
   <div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-xl font-black">🔐 Xác thực tăng cường cho quản trị viên</h2><p className="mt-1 max-w-3xl text-sm text-slate-400">Nhập lại mật khẩu quản trị để nhận quyền ngắn hạn gắn với đúng người dùng + phiên đăng nhập. Token thô chỉ nằm trong <code>sessionStorage</code> của tab và backend chỉ lưu SHA-256 hash.</p></div><span className={`rounded-full px-3 py-1 text-xs font-black ${active?'bg-emerald-500/15 text-emerald-300':'bg-amber-500/15 text-amber-300'}`} data-testid="step-up-status-v68">{active?`UNLOCKED · ${remaining}s`:'LOCKED'}</span></div>
   {!active?<form onSubmit={elevate} className="mt-5 flex max-w-xl flex-col gap-3 sm:flex-row"><input data-testid="step-up-password-v68" className="input flex-1" type="password" autoComplete="current-password" placeholder="Mật khẩu Quản trị viên hiện tại" value={password} onChange={e=>setPassword(e.target.value)} required/><button data-testid="step-up-unlock-v68" className="btn btn-primary" disabled={busy}>{busy?'Đang xác thực...':'Mở khóa thao tác nhạy cảm'}</button></form>:<div className="mt-5 flex flex-wrap items-center gap-3"><div className="text-sm text-emerald-300">Grant hợp lệ đến {local?.expiresAt?dateTime(local.expiresAt):'—'}.</div><button data-testid="step-up-revoke-v68" className="btn btn-secondary" onClick={revoke}>Khóa lại ngay</button></div>}
