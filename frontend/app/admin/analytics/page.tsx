  ...
  return <div data-testid="analytics-v51" className="space-y-7 pb-16">
    ...
    {!loading&&data&&<>
      ...
      {cinemaId?<section data-testid="cost-basis-v51" className="card p-5 sm:p-6">
        ...
        <div className="mt-5"><table className="w-full table-fixed text-xs sm:text-sm"><thead className="text-xs uppercase tracking-wide text-slate-500"><tr><th className="pb-3">Sản phẩm</th><th>Giá bán</th><th>Giá vốn</th><th>Độ phủ</th><th></th></tr></thead><tbody className="divide-y divide-slate-800/70">{data.concessionCostBasis.map(row=><tr key={`${row.cinemaId}-${row.productId}`}><td className="py-3 font-semibold">{concessionProductName(row.productName,language)}</td><td>{currency(row.sellingPrice)}</td><td><input data-testid={`cost-input-${row.productId}`} className="input !w-40" inputMode="decimal" placeholder="Chưa biết" value={costDraft[row.productId]??""} onChange={e=>setCostDraft(v=>({...v,[row.productId]:e.target.value}))}/></td><td>{row.costKnown?<span className="text-emerald-300">Đã có cost</span>:<span className="text-amber-300">Chưa biết</span>}</td><td className=""><button data-testid={`save-cost-${row.productId}`} className="btn btn-secondary" type="button" disabled={savingCost===row.productId} onClick={()=>saveCostBasis(row)}>{savingCost===row.productId?"Đang lưu...":"Lưu cost"}</button></td></tr>)}</tbody></table></div>
        ...
        <section className="card p-5 sm:p-6">
          ...
          <div className="mt-5">
            <table className="w-full table-fixed text-xs sm:text-sm">
              <thead className="text-xs uppercase tracking-wide text-slate-500"><tr><th className="pb-3">Rạp</th><th>Doanh thu</th><th>Đặt vé</th><th>Vé</th><th>Sức chứa</th><th>Lấp đầy</th></tr></thead>
      ...
      <section data-testid="analytics-snapshots-v51" className="card p-5 sm:p-6">
        ...
        <div className="mt-5 overflow-x-auto"><table data-testid="analytics-snapshot-table-v7819-r2" className="w-full min-w-[980px] text-xs sm:text-sm"><thead className="text-xs uppercase tracking-wide text-slate-500"><tr><th className="cb-table-period pb-3">Kỳ</th><th className="cb-table-cinema">Rạp</th><th className="cb-table-range">Khoảng</th><th>Doanh thu</th><th>Biên lợi nhuận</th><th>Tỷ lệ lấp đầy</th><th>Độ phủ giá vốn</th><th>Dự báo 7 ngày</th></tr></thead><tbody className="divide-y divide-slate-800/70">{data.snapshots.slice(0,18).map(x=><tr key={x.id}><td className="cb-table-period py-3 font-black">{x.periodKind}</td><td className="cb-table-cinema">{x.cinemaName}</td><td className="cb-table-range">{x.periodStart} → {x.periodEnd}</td><td>{currency(x.revenue)}</td><td>{moneyOrUnknown(x.grossMargin)}</td><td>{pct(x.occupancyRate)}</td><td>{pct(x.costCoverageRate)}</td><td>{currency(x.forecastNext7d)}</td></tr>)}</tbody></table>{!data.snapshots.length&&<p className="py-6 text-center text-slate-500">Chưa có ảnh chụp dữ liệu. Bộ lập lịch sẽ tạo dữ liệu khi backend chạy với Flyway V51.</p>}</div>
      </section>
