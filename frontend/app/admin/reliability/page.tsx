...
import { reliabilityIncidentPresentation } from "@/lib/controlled-business-presentation";
...
export default function ReliabilityV74Page(){
  ...
  return <div className="space-y-7" data-testid="reliability-v74">
    ...
    <section className="grid gap-5 lg:grid-cols-2" data-testid="burn-rate-v74">
      <BurnCard title={t("Tiêu hao nhanh","Fast burn")} window={summary?.fastWindow} language={language} t={t}/>
      <BurnCard title={t("Tiêu hao chậm","Slow burn")} window={summary?.slowWindow} language={language} t={t}/>
    </section>

    <section className="card p-5" data-testid="reliability-policy-v74">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-xl font-bold">Hàng rào độ tin cậy</h2><p className="mt-1 text-sm text-slate-500">Cảnh báo nhiều cửa sổ chỉ thành NGHIÊM TRỌNG khi cả cửa sổ nhanh và chậm cùng vượt ngưỡng. Bộ đệm bị cắt bớt sẽ hiển thị MỘT PHẦN thay vì ĐẠT giả.</p></div><Status value={summary?.dependencyStatus??"NO_DATA"} language={language}/></div>
    ...
    <section className="card overflow-hidden" data-testid="incident-timeline-v74">
      ...
      <div className="overflow-x-auto"><table className="w-full min-w-[980px] text-sm"><thead className="bg-slate-900/70 text-xs uppercase text-slate-500"><tr><th className="p-3">Thời gian</th><th className="p-3">Nguồn</th><th className="p-3">Mức nghiêm trọng</th><th className="p-3">Trạng thái</th><th className="p-3">Sự kiện</th><th className="p-3">Bằng chứng</th></tr></thead><tbody className="divide-y divide-slate-800">{incidents.map(x=>{const copy=reliabilityIncidentPresentation(x.title,x.detail,language);return <tr key={`${x.source}-${x.id}`}><td className="whitespace-nowrap p-3 text-slate-400">{dateTime(x.occurredAt)}</td><td className="min-w-28 whitespace-nowrap p-3">{x.source}</td><td className="min-w-28 whitespace-nowrap p-3"><Status value={x.severity} language={language}/></td><td className="min-w-28 whitespace-nowrap p-3">{localizedLabel(x.status,language)}</td><td className="p-3"><Link className="font-bold hover:text-cyan-300" href={x.href}>{copy.title}</Link><div className="mt-1 max-w-xl text-xs text-slate-500">{copy.detail}</div></td><td className="p-3"><code className="text-xs text-cyan-300">{x.evidenceRef}</code></td></tr>})}{incidents.length===0&&<tr><td className="p-4 text-slate-500" colSpan={6}>Không có sự cố/bằng chứng phù hợp trong cửa sổ nhìn lại.</td></tr>}</tbody></table></div>
    </section>
...
function Status({value,language}:{value:string;language:Language}){const cls=value==="CRITICAL"||value==="ACTION_REQUIRED"||value==="DEGRADED"||value==="ALERT"?"bg-rose-500/10 text-rose-300":value==="HIGH"||value==="WATCH"||value==="PARTIAL"||value==="NO_DATA"?"bg-amber-500/10 text-amber-300":"bg-emerald-500/10 text-emerald-300";return <span className={`inline-flex whitespace-nowrap rounded-full px-2.5 py-1 text-xs font-black ${cls}`}>{localizedLabel(value,language)}</span>}
function BurnCard({title,window,language,t}:{title:string;window?:ReliabilityBurnWindowV74;language:Language;t:(vi:string,en:string)=>string}){return <div className="card p-5"><div className="flex items-start justify-between gap-3"><div><h2 className="text-xl font-bold">{title}</h2><p className="mt-1 text-sm text-slate-500">{window?.windowMinutes??"-"} {t("phút","min")} · {t("ngưỡng","threshold")} {window?.alertThreshold??"-"}x</p></div><Status value={window?.status??"NO_DATA"} language={language}/></div><div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4"><Mini label={t("Yêu cầu","Requests")} value={String(window?.requests??0)}/><Mini label="5xx" value={String(window?.serverErrors??0)}/><Mini label={t("Tỷ lệ lỗi","Error rate")} value={window?`${window.errorRatePercent.toFixed(3)}%`:"-"}/><Mini label={t("Tốc độ tiêu hao","Consumption velocity")} value={window?`${window.burnRate.toFixed(3)}x`:"-"}/></div>{window?.sampleBufferTruncated&&<div className="mt-3 text-xs text-amber-300">{t("MỘT PHẦN: bộ đệm vòng mẫu đã đầy; không tuyên bố cửa sổ dữ liệu đầy đủ.","PARTIAL: the sample ring buffer is full; a complete data window is not claimed.")}</div>}</div>}
