...
import { staffJobTitleDisplay } from "@/lib/controlled-business-presentation";
  ...
  return <div className="space-y-7">
    ...
    <div className="grid gap-6 xl:grid-cols-[440px_1fr]">
      <form onSubmit={save} className="card h-fit space-y-4 p-5">
        ...
        <div>
          <label className="mb-1.5 block text-sm text-slate-300">Thư điện tử đăng nhập</label>
          <input className="input" type="email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})} required/>
          {!editingId&&<div className="mt-2 text-xs">
            {checkingEmail?<span className="text-slate-400">Đang kiểm tra thư điện tử...</span>:emailStatus?.exists?<span className={emailStatus.canPromote?"text-amber-300":"text-red-300"}>{emailStatus.message}</span>:emailStatus?<span className="text-emerald-300">✓ {emailStatus.message}</span>:<span className="text-slate-500">Thư điện tử phải là duy nhất. Nếu đã là tài khoản khách hàng, quản trị viên có thể chuyển tài khoản đó thành nhân viên.</span>}
          </div>}
        </div>
        ...
        <div><label className="mb-1.5 block text-sm text-slate-300">Chức danh</label><input className="input" placeholder={t("Nhân viên soát vé","Ticket-checking staff")} value={staffJobTitleDisplay(form.jobTitle,language)} onChange={e=>setForm({...form,jobTitle:e.target.value})}/></div>
