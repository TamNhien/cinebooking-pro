    ...
    {data&&<>
      ...
      <section className="card overflow-hidden" data-testid="fraud-risk-engine-v61">
        ...
        <table className="w-full table-fixed text-xs sm:text-sm">
          <thead className="bg-slate-950/60 text-left text-[10px] uppercase tracking-wider text-slate-500 sm:text-xs"><tr><th className="w-[24%] p-3">{t("Quy tắc","Rule")}</th><th className="w-[16%] p-3">{t("Cửa sổ","Window")}</th><th className="w-[12%] p-3 text-right">{t("Điểm tối đa","Max points")}</th><th className="p-3">{t("Giải thích","Explanation")}</th></tr></thead>
          <tbody>{data.rules.map(r=><tr key={r.code} className="border-t border-slate-800 align-top"><td className="break-words p-3"><b>{localizedLabel(r.code,language)}</b><div className="font-mono text-[10px] text-slate-600">{r.code}</div></td><td className="break-words p-3">{r.window}</td><td className="p-3 text-right font-black">+{r.maxPoints}</td><td className="break-words p-3 text-slate-400">{riskExplanation(r.code,r.explanation,language)}</td></tr>)}</tbody>
        </table>
      </section>
