...
export default function AdminAttendancePage(){
 ...
 return <div className="space-y-7">
   ...
   <section className="card grid gap-4 p-4 lg:grid-cols-[minmax(220px,1fr)_minmax(260px,1fr)_minmax(220px,auto)] lg:items-end"><div><label className="mb-1 block text-xs text-slate-400">{t("Tháng","Month")}</label><select className="input" value={month} onChange={e=>setMonth(e.target.value)}>{months.map(m=><option key={m.value} value={m.value}>{m.label}</option>)}</select></div><div><label className="mb-1 block text-xs text-slate-400">{t("Rạp","Cinema")}</label><select className="input" value={cinemaId} onChange={e=>setCinemaId(e.target.value)} disabled={role==="MANAGER"}><option value="">{t("Tất cả rạp","All cinemas")}</option>{cinemas.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></div><div className="flex items-end"><div className="text-sm text-slate-400">{t("Bảng công","Timesheet")}: <b className="text-white">{report?.cinemaName||"—"}</b></div></div></section>
