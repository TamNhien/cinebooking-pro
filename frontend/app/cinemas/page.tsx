...
const day=(v:string,locale:string)=>new Intl.DateTimeFormat(locale,{weekday:"short",day:"2-digit",month:"2-digit"}).format(new Date(`${v}T00:00:00`));
      ...
      <section>
        ...
        {dates.length>0&&<div className="card mb-5 space-y-4 p-4">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div className="flex flex-wrap gap-2">{months.map(m=><button key={m} type="button" className={`tab-pill ${month===m?"active":""}`} onClick={()=>chooseMonth(m)}>{monthLabel(m,locale)}</button>)}</div>
          ...
          <div className="flex gap-2 overflow-x-auto pb-2">{monthDates.map(d=><button key={d} onClick={()=>chooseDate(d)} className={`date-chip shrink-0 ${date===d?"active":""}`}>{day(d,locale)}</button>)}</div>
