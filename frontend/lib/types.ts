...
export type PaymentStart = { paymentId:string; bookingId:string; provider:string; paymentUrl:string; qrData?:string; deeplink?:string; expiresAt?:string; replayed:boolean };
export type PaymentCheckout = { paymentId:string; bookingId:string; provider:string; status:string; amount:number; paymentUrl?:string; qrData?:string; deeplink?:string; providerOrderId?:string; providerTransactionId?:string; providerResponseCode?:string; providerMessage?:string; createdAt:string; updatedAt:string; expiresAt?:string; paidAt?:string; failedAt?:string };
export type PaymentHistoryItem = { paymentId:string; bookingId:string; payerUserId:string; movieTitle:string; provider:string; status:string; amount:number; refundedAmount?:number; refundReference?:string; providerOrderId?:string; providerTransactionId?:string; providerResponseCode?:string; providerMessage?:string; createdAt:string; updatedAt:string; expiresAt?:string; paidAt?:string; failedAt?:string; refundedAt?:string };
export type PaymentProviderAvailability = { provider:string; configured:boolean; mock:boolean; mode:string };
export type PaymentAdminView = { id:string; bookingId:string; payerUserId:string; provider:string; status:string; amount:number; providerOrderId?:string; providerTransactionId?:string; responseCode?:string; message?:string; createdAt:string; updatedAt:string; expiresAt?:string; paidAt?:string; failedAt?:string; lastWebhookAt?:string };
...
export type PaymentOpsDashboard = { total:number; pending:number; success:number; failed:number; expired:number; review:number; refunded:number; invalidWebhooks:number; webhookEvents:number; payments:PaymentAdminView[]; webhooks:PaymentWebhookAdminView[] };
export type PaymentReconciliationResult = { paymentId:string; provider:string; localStatus:string; providerStatus:string; providerTransactionId?:string; message:string; changed:boolean };
...
export type InventoryMovement = { id:string; productId:string; productName:string; bookingId?:string; movementType:"RESTOCK"|"ADJUSTMENT"|"RESERVE"|"RELEASE"|"SALE"|"REFUND"|"LOYALTY_REWARD"; quantityDelta:number; reservedDelta:number; stockAfter:number; reservedAfter:number; actorEmail?:string; note?:string; createdAt:string };
