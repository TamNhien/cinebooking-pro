...
test("V62 admin simulates explainable bounded dynamic pricing without writing fake bookings",async({page})=>{
  ...
  await expect(page.getByTestId("dynamic-pricing-v62")).toContainText("Định giá động 4.0");
  ...
  await expect(strategy).toContainText("Thời gian");
  ...
  await expect(simulator).toContainText("không ghi cơ sở dữ liệu");
